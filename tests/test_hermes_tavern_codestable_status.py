...
CURRENT_STATUS_PREFIX = "Current status (2026-06-15): All phases 1-250 accepted"
STALE_STATUS_PREFIX = "Current status (2026-06-14): All phases 1-249 accepted"
STALE_PHASE_MARKER = "1-249"
STALE_PHASE_MARKER_EN_DASH = "1–249"
...
FINAL_STATUS_SUFFIX = "Phase 248 attention status sync through Phase 247, Phase 249 attention status sync through Phase 248, and Phase 250 attention status sync through Phase 249."
...
def test_attention_current_status_line_is_current():
    ...
    stale_aggregate_range_not_literal = "".join(["range(168, ", "250", ")"])
    assert stale_aggregate_range_not_literal not in test
    stale_phase_range = "".join(["range(168, ", "250", ")"])
    assert "range(168, 251)" in test
