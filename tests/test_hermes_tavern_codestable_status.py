...
def test_attention_current_status_line_is_current():
    ...
    assert "Current status (2026-06-13): All phases 1-193 accepted" in status
    ...
    assert "Current status (2026-06-12): All phases 1-192 accepted" not in status
    assert "All phases 1-192 accepted" not in status
    assert re.search(r"(?<!\d)1-192(?!\d)", status) is None
    assert re.search(r"(?<!\d)1–192(?!\d)", status) is None
    ...
    assert all(f"Phase {phase} " in status for phase in range(168, 194))
    assert re.search(r"(?<!\d)Phase 121-167(?!\d)", status) is not None

    test = Path(__file__).read_text(encoding="utf-8")
    assert "Current status (2026-06-13): All phases 1-193 accepted" in test
    assert "All phases 1-192 accepted" in test
    assert "1-192" in test
    assert "1–192" in test
    forbidden_glob = "." + "glob("
