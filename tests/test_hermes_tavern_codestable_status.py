...
STALE_PHASE_MARKER = "1-268"
STALE_PHASE_MARKER_EN_DASH = "1–268"
...
FINAL_STATUS_SUFFIX = "Phase 267 attention status sync through Phase 266, Phase 268 attention status sync through Phase 267, and Phase 269 attention status sync through Phase 268."
...
def test_attention_current_status_line_is_current():
    ...
    stale_aggregate_range_not_literal = "".join(["range(168, ", "26", "9", ")"])
    assert stale_aggregate_range_not_literal not in test
    aggregate_range = "range(168, 270)"
    assert aggregate_range in test
    stale_aggregate_range = "".join(["range(168, ", "26", "9", ")"])
