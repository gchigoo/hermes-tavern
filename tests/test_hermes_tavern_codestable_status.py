...
FINAL_STATUS_SUFFIX = "Phase 274 attention status sync through Phase 273, Phase 275 attention status sync through Phase 274, and Phase 276 attention status sync through Phase 275."
...
def test_attention_current_status_line_is_current():
    ...
    stale_aggregate_range_not_literal = "".join(["range(168, ", "27", "6", ")"])
    assert stale_aggregate_range_not_literal not in test
    aggregate_range = "range(168, 277)"
    assert aggregate_range in test
    stale_aggregate_range = "".join(["range(168, ", "27", "6", ")"])
