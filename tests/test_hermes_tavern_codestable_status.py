...
CURRENT_STATUS_PREFIX = "Current status (2026-06-15): All phases 1-256 accepted"
STALE_STATUS_PREFIX = "Current status (2026-06-15): All phases 1-255 accepted"
STALE_PHASE_MARKER = "1-255"
STALE_PHASE_MARKER_EN_DASH = "1–255"
...
FINAL_STATUS_SUFFIX = "Phase 254 attention status sync through Phase 253, Phase 255 attention status sync through Phase 254, and Phase 256 attention status sync through Phase 255."
...
def test_attention_current_status_line_is_current():
    ...
    stale_aggregate_range_not_literal = "".join(["range(168, ", "25", "6", ")"])
    assert stale_aggregate_range_not_literal not in test
    aggregate_range = "".join(["range(168, ", "257", ")"])
    assert aggregate_range in test
    stale_phase_range = "".join(["range(168, ", "25", "6", ")"])
