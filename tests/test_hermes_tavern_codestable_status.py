...
CURRENT_STATUS_PREFIX = "Current status (2026-06-15): All phases 1-253 accepted"
STALE_STATUS_PREFIX = "Current status (2026-06-15): All phases 1-252 accepted"
STALE_PHASE_MARKER = "1-252"
STALE_PHASE_MARKER_EN_DASH = "1–252"
...
FINAL_STATUS_SUFFIX = "Phase 251 attention status sync through Phase 250, Phase 252 attention status sync through Phase 251, and Phase 253 attention status sync through Phase 252."
...
def test_attention_current_status_line_is_current():
    ...
    stale_aggregate_range_not_literal = "".join(["range(168, ", "253", ")"])
    assert stale_aggregate_range_not_literal not in test
    aggregate_range = "".join(["range(168, ", "254", ")"])
    assert aggregate_range in test
    stale_phase_range = "".join(["range(168, ", "253", ")"])
