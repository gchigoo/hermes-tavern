...
CURRENT_STATUS_PREFIX = "Current status (2026-06-15): All phases 1-254 accepted"
STALE_STATUS_PREFIX = "Current status (2026-06-15): All phases 1-253 accepted"
STALE_PHASE_MARKER = "1-253"
STALE_PHASE_MARKER_EN_DASH = "1–253"
...
FINAL_STATUS_SUFFIX = "Phase 252 attention status sync through Phase 251, Phase 253 attention status sync through Phase 252, and Phase 254 attention status sync through Phase 253."
...
def test_attention_current_status_line_is_current():
    ...
    stale_aggregate_range_not_literal = "".join(["range(168, ", "254", ")"])
    assert stale_aggregate_range_not_literal not in test
    aggregate_range = "".join(["range(168, ", "255", ")"])
    assert aggregate_range in test
    stale_phase_range = "".join(["range(168, ", "254", ")"])
