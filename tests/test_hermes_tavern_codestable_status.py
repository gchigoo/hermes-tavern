...
CURRENT_STATUS_PREFIX = "Current status (2026-06-15): All phases 1-262 accepted"
STALE_STATUS_PREFIX = "Current status (2026-06-15): All phases 1-261 accepted"
STALE_PHASE_MARKER = "1-261"
STALE_PHASE_MARKER_EN_DASH = "1–261"
...
FINAL_STATUS_SUFFIX = "Phase 260 attention status sync through Phase 259, Phase 261 attention status sync through Phase 260, and Phase 262 attention status sync through Phase 261."
...
def test_attention_current_status_line_is_current():
    ...
    stale_aggregate_range_not_literal = "".join(["range(168, ", "26", "2", ")"])
    assert stale_aggregate_range_not_literal not in test
    aggregate_range = "range(168, 263)"
    assert aggregate_range in test
    stale_aggregate_range = "".join(["range(168, ", "26", "2", ")"])
