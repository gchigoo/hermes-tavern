...
CURRENT_STATUS_PREFIX = "Current status (2026-06-15): All phases 1-260 accepted"
STALE_STATUS_PREFIX = "Current status (2026-06-15): All phases 1-259 accepted"
STALE_PHASE_MARKER = "1-259"
STALE_PHASE_MARKER_EN_DASH = "1–259"
...
FINAL_STATUS_SUFFIX = "Phase 258 attention status sync through Phase 257, Phase 259 attention status sync through Phase 258, and Phase 260 attention status sync through Phase 259."
...
def test_attention_current_status_line_is_current():
    ...
    stale_aggregate_range_not_literal = "".join(["range(168, ", "2", "60", ")"])
    assert stale_aggregate_range_not_literal not in test
    aggregate_range = "range(168, 261)"
    assert aggregate_range in test
    stale_aggregate_range = "".join(["range(168, ", "2", "60", ")"])
