...
FINAL_STATUS_SUFFIX = "Phase 271 attention status sync through Phase 270, Phase 272 attention status sync through Phase 271, and Phase 273 attention status sync through Phase 272."
...
def test_attention_current_status_line_is_current():
    ...
    stale_aggregate_range_not_literal = "".join(["range(168, ", "27", "3", ")"])
    assert stale_aggregate_range_not_literal not in test
    aggregate_range = "range(168, 274)"
    assert aggregate_range in test
    stale_aggregate_range = "".join(["range(168, ", "27", "3", ")"])
