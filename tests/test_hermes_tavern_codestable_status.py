...
FINAL_STATUS_SUFFIX = "Phase 273 attention status sync through Phase 272, Phase 274 attention status sync through Phase 273, and Phase 275 attention status sync through Phase 274."
...
def test_attention_current_status_line_is_current():
    ...
    stale_aggregate_range_not_literal = "".join(["range(168, ", "27", "5", ")"])
    assert stale_aggregate_range_not_literal not in test
    aggregate_range = "range(168, 276)"
    assert aggregate_range in test
    stale_aggregate_range = "".join(["range(168, ", "27", "5", ")"])
