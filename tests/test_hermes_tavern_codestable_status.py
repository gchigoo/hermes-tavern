...
def test_attention_current_status_line_is_current():
    ...
    assert "Current status (2026-06-12): All phases 1-190 accepted" in status
    ...
    assert "Current status (2026-06-12): All phases 1-189 accepted" not in status

    assert re.search(r"(?<!\d)1-189(?!\d)", status) is None
    assert re.search(r"(?<!\d)1–189(?!\d)", status) is None
