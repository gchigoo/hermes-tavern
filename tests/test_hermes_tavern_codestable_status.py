...
STALE_PHASE_MARKER = "1-269"
STALE_PHASE_MARKER_EN_DASH = "1–269"
...
FINAL_STATUS_SUFFIX = "Phase 268 attention status sync through Phase 267, Phase 269 attention status sync through Phase 268, and Phase 270 attention status sync through Phase 269."
...
def test_attention_current_status_line_is_current():
    ...
    stale_aggregate_range_not_literal = "".join(["range(168, ", "27", "0", ")"])
    assert stale_aggregate_range_not_literal not in test
    aggregate_range = "range(168, 271)"
    assert aggregate_range in test
    stale_aggregate_range = "".join(["range(168, ", "27", "0", ")"])
