...
CURRENT_STATUS_PREFIX = "Current status (2026-06-16): All phases 1-282 accepted"
STALE_STATUS_PREFIX = "Current status (2026-06-16): All phases 1-281 accepted"
STALE_PHASE_MARKER = "1-281"
STALE_PHASE_MARKER_EN_DASH = "1–281"
...
FINAL_STATUS_SUFFIX = "Phase 280 attention status sync through Phase 279, Phase 281 attention status sync through Phase 280, and Phase 282 attention status sync through Phase 281."
...
def test_attention_current_status_line_is_current():
    ...
    stale_aggregate_range_not_literal = "".join(["range(168, ", "28", "0", ")"])
    assert stale_aggregate_range_not_literal not in test
    aggregate_range = "range(168, 283)"
    assert aggregate_range in test
    stale_aggregate_range = "".join(["range(168, ", "28", "1", ")"])
