...
STALE_PHASE_MARKER = "1-267"
STALE_PHASE_MARKER_EN_DASH = "1–267"
...
FINAL_STATUS_SUFFIX = "Phase 266 attention status sync through Phase 265, Phase 267 attention status sync through Phase 266, and Phase 268 attention status sync through Phase 267."
...
def test_attention_current_status_line_is_current():
    ...
    stale_aggregate_range_not_literal = "".join(["range(168, ", "26", "8", ")"])
    assert stale_aggregate_range_not_literal not in test
    aggregate_range = "range(168, 269)"
    assert aggregate_range in test
    stale_aggregate_range = "".join(["range(168, ", "26", "8", ")"])
