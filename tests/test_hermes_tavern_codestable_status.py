...
CURRENT_STATUS_PREFIX = "Current status (2026-06-15): All phases 1-266 accepted"
STALE_STATUS_PREFIX = "Current status (2026-06-15): All phases 1-265 accepted"
STALE_PHASE_MARKER = "1-265"
STALE_PHASE_MARKER_EN_DASH = "1–265"
...
FINAL_STATUS_SUFFIX = "Phase 264 attention status sync through Phase 263, Phase 265 attention status sync through Phase 264, and Phase 266 attention status sync through Phase 265."
...
def test_attention_current_status_line_is_current():
    ...
    stale_aggregate_range_not_literal = "".join(["range(168, ", "26", "6", ")"])
    assert stale_aggregate_range_not_literal not in test
    aggregate_range = "range(168, 267)"
    assert aggregate_range in test
    stale_aggregate_range = "".join(["range(168, ", "26", "6", ")"])
    assert stale_aggregate_range not in test
