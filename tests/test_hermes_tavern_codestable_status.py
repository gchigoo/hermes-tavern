...
CURRENT_STATUS_PREFIX = "Current status (2026-06-15): All phases 1-257 accepted"
STALE_STATUS_PREFIX = "Current status (2026-06-15): All phases 1-256 accepted"
STALE_PHASE_MARKER = "1-256"
STALE_PHASE_MARKER_EN_DASH = "1–256"
...
FINAL_STATUS_SUFFIX = "Phase 255 attention status sync through Phase 254, Phase 256 attention status sync through Phase 255, and Phase 257 attention status sync through Phase 256."
...
def test_attention_current_status_line_is_current():
    ...
    stale_aggregate_range_not_literal = "".join(["range(168, ", "25", "7", ")"])
    assert stale_aggregate_range_not_literal not in test
    aggregate_range = "range(168, 258)"
    assert aggregate_range in test
    stale_aggregate_range = "".join(["range(168, ", "25", "7", ")"])
