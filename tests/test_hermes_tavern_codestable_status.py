...
PHASE_517_CHECKLIST = REPO_ROOT / "design/codestable/features/2026-07-24-hermes-tavern-phase517-attention-status-sync-through-phase516/2026-07-24-hermes-tavern-phase517-attention-status-sync-through-phase516-checklist.yaml"
PHASE_518_DIR = REPO_ROOT / "design/codestable/features/2026-07-24-hermes-tavern-phase518-phase517-lifecycle-coverage-parity"
...
PHASE_522_DIR = REPO_ROOT / "design/codestable/features/2026-07-29-hermes-tavern-phase522-attention-status-sync-through-phase521"
PHASE_522_DESIGN = PHASE_522_DIR / "2026-07-29-hermes-tavern-phase522-attention-status-sync-through-phase521-design.md"
PHASE_522_CHECKLIST = PHASE_522_DIR / "2026-07-29-hermes-tavern-phase522-attention-status-sync-through-phase521-checklist.yaml"
PHASE_522_ACCEPTANCE = PHASE_522_DIR / "2026-07-29-hermes-tavern-phase522-attention-status-sync-through-phase521-acceptance.md"
PHASE_523_DIR = REPO_ROOT / "design/codestable/features/2026-07-29-hermes-tavern-phase523-phase522-lifecycle-record-reconciliation"
...
PHASE_527_STATUS_LABEL = "Phase 527 attention status sync through Phase 526"
PHASE_528_STATUS_LABEL = "Phase 528 attention status sync through Phase 527"
...
CURRENT_STATUS_PREFIX = "Current status (2026-06-18): All phases 1-528 accepted"
STALE_STATUS_PREFIX = "Current status (2026-06-18): All phases 1-527 accepted"
STALE_PHASE_MARKER = "1-527"
STALE_PHASE_MARKER_EN_DASH = "1–527"
PREVIOUS_STALE_STATUS_PREFIX = "Current status (2026-06-18): All phases 1-526 accepted"
PREVIOUS_STALE_PHASE_MARKER = "1-526"
PREVIOUS_STALE_PHASE_MARKER_EN_DASH = "1–526"
...
REQUIRED_PHASE_LABELS = [
    ...
    "Phase 488 CodeStable verification command parity",
    ...
    "Phase 500 attention status sync through Phase 499",
    "Phase 501 attention status sync through Phase 500",
    "Phase 502 attention status sync through Phase 501",
    "Phase 503 attention status sync through Phase 502",
    "Phase 504 attention status sync through Phase 503",
    "Phase 505 attention status sync through Phase 504",
    "Phase 506 attention status sync through Phase 505",
    "Phase 507 attention status sync through Phase 506",
    "Phase 508 attention status sync through Phase 507",
    ...
    "Phase 526 phase525 acceptance reference integrity",
    "Phase 527 attention status sync through Phase 526",
    "Phase 528 attention status sync through Phase 527",
]
FINAL_STATUS_SUFFIX = "Phase 516 attention status sync through Phase 515, Phase 517 attention status sync through Phase 516, Phase 518 phase517 lifecycle coverage parity, Phase 519 phase518 lifecycle record reconciliation, Phase 520 canonical design-plan path parity, Phase 521 attention status sync through Phase 520, Phase 522 attention status sync through Phase 521, Phase 523 phase522 lifecycle record reconciliation, Phase 524 phase523 lifecycle record reconciliation, Phase 525 phase524 post-commit record plan, Phase 526 phase525 acceptance reference integrity, Phase 527 attention status sync through Phase 526, and Phase 528 attention status sync through Phase 527."
...
def test_status_sync_checklists_have_expected_lifecycle_contract():
    ...
    phase_515_checklist = yaml.safe_load(PHASE_515_CHECKLIST.read_text(encoding="utf-8"))
    assert phase_515_checklist["status"] == "accepted"
    assert all(step.get("status") == "done" for step in phase_515_checklist["steps"])
    assert all(check.get("status") == "passed" for check in phase_515_checklist["checks"])
    phase_515_steps = {step["id"]: step for step in phase_515_checklist["steps"]}
    assert phase_515_steps["controller-verification"]["status"] == "done"
    controller_evidence = phase_515_steps["controller-verification"].get("evidence")
    assert isinstance(controller_evidence, str) and controller_evidence.strip()

    phase_516_checklist = yaml.safe_load(PHASE_516_CHECKLIST.read_text(encoding="utf-8"))
    assert phase_516_checklist["status"] == "accepted"
    assert all(step.get("status") == "done" for step in phase_516_checklist["steps"])
    assert all(check.get("status") == "passed" for check in phase_516_checklist["checks"])
    phase_516_steps = {step["id"]: step for step in phase_516_checklist["steps"]}
    controller_evidence = phase_516_steps["controller-verification"].get("evidence")
    assert isinstance(controller_evidence, str) and controller_evidence.strip()

    phase_517_checklist = yaml.safe_load(PHASE_517_CHECKLIST.read_text(encoding="utf-8"))
    assert phase_517_checklist["status"] == "accepted"
    assert all(step.get("status") == "done" for step in phase_517_checklist["steps"])
    assert all(check.get("status") == "passed" for check in phase_517_checklist["checks"])
    phase_517_steps = {step["id"]: step for step in phase_517_checklist["steps"]}
    controller_evidence = phase_517_steps["controller-verification"].get("evidence")
    assert isinstance(controller_evidence, str) and controller_evidence.strip()

    phase_518_design_text = PHASE_518_DESIGN.read_text(encoding="utf-8")
    phase_518_checklist_text = PHASE_518_CHECKLIST.read_text(encoding="utf-8")
    phase_518_design = yaml.safe_load(phase_518_design_text.split("---", 2)[1])
    phase_518_checklist = yaml.safe_load(phase_518_checklist_text)
    phase_518_acceptance = yaml.safe_load(
        PHASE_518_ACCEPTANCE.read_text(encoding="utf-8").split("---", 2)[1]
    )

    expected_phase_518_feature = "2026-07-24-hermes-tavern-phase518-phase517-lifecycle-coverage-parity"
    assert {
        phase_518_design["feature"],
        phase_518_checklist["feature"],
        phase_518_acceptance["feature"],
    } == {expected_phase_518_feature}
    assert phase_518_design["status"] == "approved"
    assert phase_518_design["parent_verification_status"] == "verified"
    assert phase_518_design["acceptance_state"] == "accepted"
    assert phase_518_checklist["status"] == phase_518_acceptance["status"] == "accepted"
    assert phase_518_acceptance["doc_type"] == "feature-acceptance"
    assert all(step.get("status") == "done" for step in phase_518_checklist["steps"])
    assert all(check.get("status") == "passed" for check in phase_518_checklist["checks"])
...
def test_phase522_accepted_lifecycle_records_are_consistent():
    phase_522_design_text = PHASE_522_DESIGN.read_text(encoding="utf-8")
    phase_522_checklist_text = PHASE_522_CHECKLIST.read_text(encoding="utf-8")
    phase_522_acceptance_text = PHASE_522_ACCEPTANCE.read_text(encoding="utf-8")
    phase_522_design = yaml.safe_load(phase_522_design_text.split("---", 2)[1])
    phase_522_checklist = yaml.safe_load(phase_522_checklist_text)
    phase_522_acceptance = yaml.safe_load(phase_522_acceptance_text.split("---", 2)[1])

    expected_feature = "2026-07-29-hermes-tavern-phase522-attention-status-sync-through-phase521"
    assert {
        phase_522_design["feature"],
        phase_522_checklist["feature"],
        phase_522_acceptance["feature"],
    } == {expected_feature}
    assert phase_522_design["status"] == "approved"
    assert phase_522_design["parent_verification_status"] == "completed"
    assert phase_522_design["acceptance_state"] == "accepted"
    assert phase_522_checklist["status"] == "accepted"
    assert phase_522_checklist["workflow_status"] == "accepted"
    assert phase_522_checklist["acceptance_state"] == "accepted"
    assert phase_522_checklist["parent_verification_completed"] is True
    assert phase_522_checklist["acceptance_md_present"] is True
    assert phase_522_acceptance["doc_type"] == "feature-acceptance"
    assert phase_522_acceptance["status"] == "accepted"
    assert phase_522_acceptance["audit_state"] == "passed"
    assert phase_522_acceptance["controller_verification_state"] == "completed"
    assert all(step.get("status") == "done" for step in phase_522_checklist["steps"])
    assert all(check.get("status") == "passed" for check in phase_522_checklist["checks"])

    phase_522_history = "\n".join(
        (phase_522_design_text, phase_522_checklist_text, phase_522_acceptance_text)
    ).lower()
    for artifact_text in (
        phase_522_design_text,
        phase_522_checklist_text,
        phase_522_acceptance_text,
    ):
        assert "final policy-aligned architect" in artifact_text.lower()
        assert "returned pass" in artifact_text.lower()
    for stale_phrase in (
        "will require a final policy-aligned architect pass before committing",
        "remains the commit gate",
        "before commit",
        "commit and push require the final policy-aligned architect pass review",
    ):
        assert stale_phrase not in phase_522_history


def test_phase523_accepted_lifecycle_records_are_consistent():
    phase_523_design_text = PHASE_523_DESIGN.read_text(encoding="utf-8")
    phase_523_checklist_text = PHASE_523_CHECKLIST.read_text(encoding="utf-8")
    phase_523_acceptance_text = PHASE_523_ACCEPTANCE.read_text(encoding="utf-8")
    phase_523_design = yaml.safe_load(phase_523_design_text.split("---", 2)[1])
    phase_523_checklist = yaml.safe_load(phase_523_checklist_text)
    phase_523_acceptance = yaml.safe_load(phase_523_acceptance_text.split("---", 2)[1])

    expected_feature = "2026-07-29-hermes-tavern-phase523-phase522-lifecycle-record-reconciliation"
    assert {
        phase_523_design["feature"],
        phase_523_checklist["feature"],
        phase_523_acceptance["feature"],
    } == {expected_feature}
    assert phase_523_design["status"] == "approved"
    assert phase_523_design["parent_verification_completed"] is True
    assert phase_523_design["acceptance_state"] == "accepted"
    assert phase_523_checklist["status"] == "accepted"
    assert phase_523_checklist["lifecycle_status"] == "accepted"
    assert phase_523_checklist["workflow_status"] == "accepted"
    assert phase_523_checklist["acceptance_state"] == "accepted"
    assert phase_523_checklist["parent_verification_completed"] is True
    assert phase_523_checklist["controller_review_completed"] is True
    assert phase_523_checklist["worker_commit_or_push_performed"] is False
    assert phase_523_checklist["parent_commit_or_push_performed"] is True
    assert phase_523_checklist["acceptance_md_present"] is True
    assert phase_523_acceptance["doc_type"] == "feature-acceptance"
    assert phase_523_acceptance["status"] == "accepted"
    assert phase_523_acceptance["audit_state"] == "passed"
    assert phase_523_acceptance["controller_review_completed"] is True
    assert phase_523_acceptance["lifecycle_promotion"] == "completed"
    assert all(step.get("status") == "done" for step in phase_523_checklist["steps"])
...
def test_phase526_closeout_records_remain_accepted_with_canonical_status_history():
    ...
    assert status.count(PHASE_527_STATUS_LABEL) == 1
    assert status.count(PHASE_528_STATUS_LABEL) == 1
    assert "Phase " + "529" not in status
...
def test_attention_current_status_line_is_current():
    ...
    assert len(REQUIRED_PHASE_LABELS) == 361
    phase_range = range(168, 529)
    assert [int(label.split()[1]) for label in REQUIRED_PHASE_LABELS] == list(range(168, 529))
    ...
    assert PREVIOUS_STALE_STATUS_PREFIX not in status
    ...
    assert re.search(rf"(?<!\d){re.escape(PREVIOUS_STALE_PHASE_MARKER)}(?!\d)", status) is None
    assert re.search(rf"(?<!\d){re.escape(PREVIOUS_STALE_PHASE_MARKER_EN_DASH)}(?!\d)", status) is None
    ...
    assert re.search(rf"(?<!\d){re.escape(HISTORIC_STALE_PHASE_MARKER)}(?!\d)", status) is None
    assert re.search(rf"(?<!\d){re.escape(HISTORIC_STALE_PHASE_MARKER_EN_DASH)}(?!\d)", status) is None
    ...
    aggregate_range = "range(168, 529)"
    stale_aggregate_range_528 = "".join(["range(168, ", "52", "8", ")"])
    ...
    assert stale_aggregate_range_528 not in test
    ...
    stale_aggregate_range_524 = "".join(["range(168, ", "52", "4", ")"])
    stale_aggregate_range_523 = "".join(["range(168, ", "52", "3", ")"])
    ...
    stale_aggregate_range_504 = "".join(["range(168, ", "50", "4", ")"])
    stale_aggregate_range_503 = "".join(["range(168, ", "50", "3", ")"])
    stale_aggregate_range_502 = "".join(["range(168, ", "50", "2", ")"])
    stale_aggregate_range_501 = "".join(["range(168, ", "50", "1", ")"])
    stale_aggregate_range_500 = "".join(["range(168, ", "50", "0", ")"])
    stale_aggregate_range_499 = "".join(["range(168, ", "49", "9", ")"])
    stale_aggregate_range_498 = "".join(["range(168, ", "49", "8", ")"])
    stale_aggregate_range_497 = "".join(["range(168, ", "49", "7", ")"])
    stale_aggregate_range_496 = "".join(["range(168, ", "49", "6", ")"])
    ...
    stale_aggregate_range_481 = "".join(["range(168, ", "48", "1", ")"])
    stale_aggregate_range_482 = "".join(["range(168, ", "48", "2", ")"])
    stale_aggregate_range_483 = "".join(["range(168, ", "48", "3", ")"])
    stale_aggregate_range_486 = "".join(["range(168, ", "48", "6", ")"])
    stale_aggregate_range_487 = "".join(["range(168, ", "48", "7", ")"])
    stale_aggregate_range_485 = "".join(["range(168, ", "48", "5", ")"])
    stale_aggregate_range_484 = "".join(["range(168, ", "48", "4", ")"])
    ...
    for stale_range in (
        ...
        stale_aggregate_range_524,
        stale_aggregate_range_523,
        stale_aggregate_range_519,
        stale_aggregate_range_518,
        stale_aggregate_range_506,
        stale_aggregate_range_503,
        stale_aggregate_range_502,
        stale_aggregate_range_501,
        stale_aggregate_range_500,
        stale_aggregate_range_499,
        stale_aggregate_range_498,
        stale_aggregate_range_497,
        stale_aggregate_range_496,
        ...
        stale_aggregate_range_505,
        stale_aggregate_range_504,
        ...
        stale_aggregate_range_483,
        stale_aggregate_range_484,
        stale_aggregate_range_485,
        stale_aggregate_range_486,
        stale_aggregate_range_487,
        stale_aggregate_range_482,
        stale_aggregate_range_481,
    ...
    stale_aggregate_guard_508 = "".join(["range(168, ", "50", str(8), ")"])
    stale_aggregate_guard_507 = "".join(["range(168, ", "50", str(7), ")"])
    stale_aggregate_guard_506 = "".join(["range(168, ", "50", str(6), ")"])
    stale_aggregate_guard_505 = "".join(["range(168, ", "50", str(5), ")"])
    stale_aggregate_guard_504 = "".join(["range(168, ", "50", str(4), ")"])
    stale_aggregate_guard_503 = "".join(["range(168, ", "50", str(3), ")"])
    stale_aggregate_guard_502 = "".join(["range(168, ", "50", str(2), ")"])
    stale_aggregate_guard_501 = "".join(["range(168, ", "50", str(1), ")"])
    stale_aggregate_guard_500 = "".join(["range(168, ", "50", str(0), ")"])
    stale_aggregate_guard_499 = "".join(["range(168, ", "49", str(9), ")"])
    stale_aggregate_guard_498 = "".join(["range(168, ", "49", str(8), ")"])
    stale_aggregate_guard_497 = "".join(["range(168, ", "49", str(7), ")"])
    stale_aggregate_guard_496 = "".join(["range(168, ", "49", str(6), ")"])
    ...
    stale_aggregate_guard_482 = "".join(["range(168, ", "48", str(2), ")"])
    stale_aggregate_guard_483 = "".join(["range(168, ", "48", str(3), ")"])
    stale_aggregate_guard_486 = "".join(["range(168, ", "48", str(6), ")"])
    stale_aggregate_guard_487 = "".join(["range(168, ", "48", str(7), ")"])
    stale_aggregate_guard_485 = "".join(["range(168, ", "48", str(5), ")"])
    stale_aggregate_guard_484 = "".join(["range(168, ", "48", str(4), ")"])
    ...
    assert stale_aggregate_guard_508 not in test
    assert stale_aggregate_guard_507 not in test
    assert stale_aggregate_guard_506 not in test
    assert stale_aggregate_guard_505 not in test
    assert stale_aggregate_guard_504 not in test
    assert stale_aggregate_guard_503 not in test
    assert stale_aggregate_guard_502 not in test
    assert stale_aggregate_guard_501 not in test
    assert stale_aggregate_guard_500 not in test
    assert stale_aggregate_guard_499 not in test
    assert stale_aggregate_guard_498 not in test
    assert stale_aggregate_guard_497 not in test
    assert stale_aggregate_guard_496 not in test
    ...
    assert stale_aggregate_guard_488 not in test
    assert stale_aggregate_guard_487 not in test
    assert stale_aggregate_guard_486 not in test
    assert stale_aggregate_guard_485 not in test
    assert stale_aggregate_guard_484 not in test
    assert stale_aggregate_guard_483 not in test
    assert stale_aggregate_guard_482 not in test
    assert stale_aggregate_guard_481 not in test
