...
summary: >
  ...
  regression through accepted Phase 195, without changing runtime, source,
...
# Phase 196: CodeStable Attention Status Sync Through Phase 195
...
Phase 195 is accepted in the feature archive, with accepted checklist and
`phase195-acceptance.md` artifacts. The mandatory startup status and focused
...
- the current-status bullet contains `Current status (2026-06-13): All phases 1-195 accepted`;
- the bullet names Phase 168 through Phase 195 with the same short labels above;
- the bullet preserves Phase 168 through Phase 194 labels and appends `Phase 195 attention status sync through Phase 194`;
- the bullet no longer contains `All phases 1-194 accepted`;
- the bullet no longer contains standalone stale terminal `1-194` / `1–194` markers;
- the valid internal `Phase 121-167` range remains allowed;
- the final punctuation ends with `Phase 193 attention status sync through Phase 192, Phase 194 attention status sync through Phase 193, and Phase 195 attention status sync through Phase 194.`;
- the aggregate range assertion covers Phase 168 through Phase 195 with `range(168, 196)`;
...
Do not edit Phase 195 artifacts or any older accepted phase artifacts during S1.
S2 may edit only the new Phase 196 artifacts listed above.
...
- `design/codestable/attention.md` reports current status as 2026-06-13 with all
  phases 1-195 accepted.
- The current-status bullet preserves the existing Phase 168 through Phase 194
  short labels and appends `Phase 195 attention status sync through Phase 194`.
- The final list punctuation is exactly:
  `Phase 193 attention status sync through Phase 192, Phase 194 attention status sync through Phase 193, and Phase 195 attention status sync through Phase 194.`
- The current-status bullet no longer reports Phase 194 as the terminal accepted
  phase.
- The status regression rejects `All phases 1-194 accepted`.
- The status regression rejects standalone stale terminal `1-194` / `1–194`
  markers.
- The status regression allows the valid `Phase 121-167` internal range.
- The status regression explicitly asserts Phase 168 through Phase 195 labels,
  including `Phase 195 attention status sync through Phase 194`.
- The aggregate phase range assertion uses `range(168, 196)`.
...
- The stale-marker guard must reject only standalone terminal `1-194` / `1–194`
  markers while preserving the valid `Phase 121-167` internal range and the
  Phase 194 and Phase 195 short labels.
- This phase must not update Phase 195 or older accepted feature artifacts during
  S1.
