...
summary: >
  ...
  regression through accepted Phase 309 only, without changing runtime behavior.
---

# Phase 310: CodeStable Attention Status Sync Through Phase 309
...
- `design/codestable/attention.md` reports `Current status (2026-06-18): All phases 1-308 accepted`.
- `tests/test_hermes_tavern_codestable_status.py` enforces the Phase 308 prefix and `range(168, 309)`.
- No Phase 310 status-sync feature directory existed before this controller materialization.

## Goals

- Advance exactly one recurring status-sync phase: Phase 310 syncs through accepted Phase 309 only.
- Update only `attention.md`, the focused static status test, and this Phase 310 feature artifact directory.
...
- Do not add a Phase 310 attention/status label; this slice records Phase 309 as the newly accepted completed phase.
...
- From: `Current status (2026-06-18): All phases 1-308 accepted`
- To: `Current status (2026-06-18): All phases 1-309 accepted`

Append exactly once after `Phase 308 attention status sync through Phase 307`:
- `Phase 309 attention status sync through Phase 308`

Preserve:
- `Phase 121-167`
- Every explicit `Phase 168` through `Phase 308` short label exactly as already written.

Do not add:
- Any `Phase 310` attention/status label to `attention.md` or the focused status label list.

Final suffix must become exactly:
`Phase 303 attention status sync through Phase 302, Phase 304 attention status sync through Phase 303, Phase 305 attention status sync through Phase 304, Phase 306 attention status sync through Phase 305, Phase 307 attention status sync through Phase 306, Phase 308 attention status sync through Phase 307, and Phase 309 attention status sync through Phase 308.`

## Focused Test Contract

Update `tests/test_hermes_tavern_codestable_status.py`:
- `CURRENT_STATUS_PREFIX = "Current status (2026-06-18): All phases 1-309 accepted"`
- `STALE_STATUS_PREFIX = "Current status (2026-06-18): All phases 1-308 accepted"`
- `STALE_PHASE_MARKER = "1-308"`
- `STALE_PHASE_MARKER_EN_DASH = "1–308"`
- Append `Phase 309 attention status sync through Phase 308` to `REQUIRED_PHASE_LABELS` exactly once.
- Preserve every existing required label through Phase 308.
- `FINAL_STATUS_SUFFIX` equals the Phase 303/304/305/306/307/308/309 suffix above.
- Runtime `phase_range` becomes `range(168, 310)`.
- Source self-check literal becomes `aggregate_range = "range(168, 310)"`.
- Add split stale aggregate guard for `range(168, 309)`.
- Retain older split stale guards including 308, 307, 306, 305, 304, 303, 302, 301, 300, 299, 298, 297, 296, and earlier existing guards.
...
- `design/codestable/features/2026-06-22-hermes-tavern-phase310-attention-status-sync-through-phase309/design.md`
- `design/codestable/features/2026-06-22-hermes-tavern-phase310-attention-status-sync-through-phase309/checklist.yaml`
- `design/codestable/features/2026-06-22-hermes-tavern-phase310-attention-status-sync-through-phase309/acceptance.md` only as parent-pending/non-final evidence if created before parent verification.
...
- Update `attention.md` prefix and append only the Phase 309 label.
...
- `attention.md` must not contain stale terminal status `All phases 1-308 accepted` after the update.
- `attention.md` must not contain standalone stale markers `1-308` or `1–308`.
- Test source may contain stale values only as intentional `STALE_*` constants/assertions or split stale aggregate guards.
- Add split stale aggregate guard for `range(168, 309)` and retain older split guards.

## Verification Commands

- `PYTHONDONTWRITEBYTECODE=1 python design/codestable/tools/validate-yaml.py --dir design/codestable/features/2026-06-22-hermes-tavern-phase310-attention-status-sync-through-phase309 --require doc_type --require status --require feature`
...
- Adding a Phase 310 label instead of only the Phase 309 label.
- Leaving stale `1-308` markers in the attention current-status bullet.
...
Rollback point: revert only the five allowed Phase 310 files to the pre-executor state; because no staging/commit/push occurs in this lane, rollback is limited to the dirty working tree.

## Executor Prompt

Implement only Phase 310 attention/status sync through accepted Phase 309. Edit only `design/codestable/attention.md`, `tests/test_hermes_tavern_codestable_status.py`, and files inside `design/codestable/features/2026-06-22-hermes-tavern-phase310-attention-status-sync-through-phase309/`. Advance current status to `All phases 1-309 accepted`, preserve `Phase 121-167` and every explicit Phase 168 through Phase 308 label, append exactly `Phase 309 attention status sync through Phase 308`, and do not add any Phase 310 attention/status label. Update focused test constants, suffix, required labels, `range(168, 310)`, `aggregate_range = "range(168, 310)"`, stale markers for `All phases 1-308 accepted`, and split stale aggregate guard for `range(168, 309)`. Do not commit, push, stage, update state, send messages, touch runtime/protected paths, change dependencies, edit prior phase directories, or run service lifecycle commands.
