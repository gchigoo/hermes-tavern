...
summary: >
  ...
  regression through accepted Phase 189, without changing runtime, source,
...
# Phase 190: CodeStable Attention Status Sync Through Phase 189
...
Phase 189 is now accepted in the feature archive, so the mandatory startup status
...
- the current-status bullet contains `Current status (2026-06-12): All phases 1-189 accepted`;
- the bullet names Phase 168 through Phase 189 with the same short labels above;
- the bullet no longer contains standalone terminal `1-188` / `1–188` markers;
...
- `design/codestable/attention.md` reports current status as 2026-06-12 with all
  phases 1-189 accepted.
- The current-status bullet preserves the existing Phase 168 through Phase 188
  short labels and appends `Phase 189 attention status sync through Phase 188`.
- The current-status bullet no longer reports Phase 188 as the terminal accepted
  phase.
- The status regression rejects standalone terminal `1-188` / `1–188` markers.
...
- The stale-marker guard must reject only standalone terminal `1-188` / `1–188`
  markers while preserving the valid internal `Phase 121-167` range and the
  Phase 188 and Phase 189 short labels.
