...
summary: >
  ...
  regression through accepted Phase 222, without changing runtime/source behavior
...
# Phase 223: CodeStable Attention Status Sync Through Phase 222
...
Phase 222 is accepted: the Phase 222 checklist has `status: accepted` and
`workflow_status: completed`, and `phase222-acceptance.md` has
`status: accepted`.

`design/codestable/attention.md` currently reports
`Current status (2026-06-14): All phases 1-221 accepted` and ends with
`Phase 219 attention status sync through Phase 218, Phase 220 attention status
sync through Phase 219, and Phase 221 attention status sync through Phase 220.`

`tests/test_hermes_tavern_codestable_status.py` currently guards terminal range
1-221, stale 1-220 markers, Phase 168 through Phase 221 labels, and
`range(168, 222)`.

The target Phase 223 feature directory did not exist at read-only precheck time.

## Gap

`design/codestable/attention.md` is the mandatory startup read for CodeStable work
in this repository. Phase 222 is accepted in the feature archive, but the startup
status and focused static regression still stop at Phase 221. The mandatory
...
`design/codestable/features/2026-06-14-hermes-tavern-phase223-attention-status-sync-through-phase222/`

Update only the current-status bullet in `design/codestable/attention.md` so it
reports accepted phases through Phase 222. Advance only the terminal range to
`All phases 1-222 accepted`, preserve prior labels, adjust the final-list
...
Do not edit Phase 222 artifacts or any older accepted phase artifacts during S1.
S2 may edit only the new Phase 223 artifacts listed above.

S1 must not mark final accepted statuses and must not create
`phase223-acceptance.md`. S2 is controller-only acceptance closeout.
...
Phase 1-222 terminal status, Phase 168-222 explicit labels, stale 1-221
rejection, valid Phase 121-167 preservation, final-list punctuation, and
`range(168, 223)` aggregate assertion.

S2 is controller-only closeout. It records acceptance evidence in
`phase223-acceptance.md` and finalizes the Phase 223 checklist/design statuses
...
- `PYTHONDONTWRITEBYTECODE=1 python design/codestable/tools/validate-yaml.py --file design/codestable/features/2026-06-14-hermes-tavern-phase223-attention-status-sync-through-phase222/design.md --require doc_type --require status --require feature --require implementation_ready`
- `PYTHONDONTWRITEBYTECODE=1 python design/codestable/tools/validate-yaml.py --file design/codestable/features/2026-06-14-hermes-tavern-phase223-attention-status-sync-through-phase222/checklist.yaml --yaml-only --require doc_type --require status --require feature --require implementation_ready`
...
- static marker guard must confirm: exactly one current-status line, terminal `1-222`, explicit Phase 168 through Phase 222 labels, `range(168, 223)`, final punctuation through Phase 222, stale terminal `1-221` / `1–221` rejection, valid `Phase 121-167` preservation, and no generated phase-discovery calls.
...
Phase 223 is acceptable when:

- the single current-status bullet reports all phases 1-222 accepted;
- the status bullet preserves prior labels through Phase 221 and appends only `Phase 222 attention status sync through Phase 221`;
- the final suffix is exactly `Phase 220 attention status sync through Phase 219, Phase 221 attention status sync through Phase 220, and Phase 222 attention status sync through Phase 221.`;
- stale terminal 1-221 markers are rejected while valid internal `Phase 121-167` remains allowed;
- `REQUIRED_PHASE_LABELS` includes Phase 168 through Phase 222;
- the aggregate assertion is `range(168, 223)`;
...
- The stale-negative checks and aggregate range must advance together: stale
  prefixes/markers move from 1-221 while the aggregate assertion moves to
  `range(168, 223)`.
...
- The final-list punctuation must move the `and` from Phase 221 to Phase 222
