...
summary: >
  ...
  regression through accepted Phase 187, without changing runtime, source,
...
# Phase 188: CodeStable Attention Status Sync Through Phase 187
...
Phase 187 is now accepted in the feature archive, so the mandatory startup status
...
- the current-status bullet contains `Current status (2026-06-12): All phases 1-187 accepted`;
- the bullet names Phase 168 through Phase 187 with the same short labels above;
- the bullet no longer contains standalone terminal `1-186` / `1–186` markers;
...
- `design/codestable/attention.md` reports current status as 2026-06-12 with all
  phases 1-187 accepted.
- The current-status bullet explicitly includes Phase 168 through Phase 187 short
  labels.
- The current-status bullet no longer reports Phase 186 as the terminal accepted
  phase.
- The status regression rejects standalone terminal `1-186` / `1–186` markers.
...
- The stale-marker guard must reject only standalone terminal `1-186` / `1–186`
  markers while preserving the valid internal `Phase 121-167` range and the
  Phase 186 and Phase 187 short labels.
