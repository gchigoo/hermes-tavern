...
summary: >
  ...
  regression through accepted Phase 214, without changing runtime, source,
...
# Phase 215: CodeStable Attention Status Sync Through Phase 214
...
1-213 accepted` and ends with `Phase 213 attention status sync through Phase
212`.

Phase 214 is accepted in the feature archive:
...
`phase214-acceptance.md` has `status: accepted`. The focused status regression
currently guards Phase 1-213 with stale 1-212 markers and `range(168, 214)`. The
...
- the current-status bullet contains `Current status (2026-06-13): All phases 1-214 accepted`;
- the bullet names Phase 168 through Phase 214 with the same short labels above;
- the bullet preserves Phase 168 through Phase 213 labels and appends `Phase 214 attention status sync through Phase 213`;
- the bullet no longer contains `All phases 1-213 accepted`;
- the bullet no longer contains standalone stale terminal `1-213` / `1–213` markers;
- the valid `Phase 121-167` internal range remains allowed;
- the final punctuation ends with `Phase 212 attention status sync through Phase 211, Phase 213 attention status sync through Phase 212, and Phase 214 attention status sync through Phase 213.`;
- the aggregate phase range assertion covers Phase 168 through Phase 214 with `range(168, 215)`;
...
Do not edit Phase 214 artifacts or any older accepted phase artifacts during S1.
S2 may edit only the new Phase 215 artifacts listed above.
...
Phase 1-214 terminal status, Phase 168-214 explicit labels, stale 1-213
rejection, valid Phase 121-167 preservation, final-list punctuation, and
`range(168, 215)` aggregate assertion.

S2 is controller-only closeout. It records acceptance evidence in
`phase215-acceptance.md` and finalizes the Phase 215 design/checklist statuses
...
- `PYTHONDONTWRITEBYTECODE=1 python design/codestable/tools/validate-yaml.py --file design/codestable/features/2026-06-13-hermes-tavern-phase215-attention-status-sync-through-phase214/design.md --require doc_type --require status --require feature --require implementation_ready`
- `PYTHONDONTWRITEBYTECODE=1 python design/codestable/tools/validate-yaml.py --file design/codestable/features/2026-06-13-hermes-tavern-phase215-attention-status-sync-through-phase214/checklist.yaml --yaml-only --require doc_type --require status --require feature --require implementation_ready`
...
- static marker guard must confirm: exactly one current-status line, terminal `1-214`, explicit Phase 168 through Phase 214 labels, `range(168, 215)`, final punctuation through Phase 214, stale terminal `1-213` / `1–213` rejection, valid `Phase 121-167` preservation, and no generated phase-discovery calls.
...
- `design/codestable/attention.md` reports current status as 2026-06-13 with all
  phases 1-214 accepted.
- The current-status bullet preserves the existing Phase 168 through Phase 213
  short labels and appends `Phase 214 attention status sync through Phase 213`.
- The final list punctuation is exactly:
  `Phase 212 attention status sync through Phase 211, Phase 213 attention status sync through Phase 212, and Phase 214 attention status sync through Phase 213.`
- The current-status bullet no longer reports Phase 213 as the terminal accepted
  phase.
- The status regression rejects `All phases 1-213 accepted`.
- The status regression rejects standalone stale terminal `1-213` / `1–213`
  markers.
- The status regression allows the valid `Phase 121-167` internal range.
- The status regression explicitly asserts Phase 168 through Phase 214 labels,
  including `Phase 213 attention status sync through Phase 212` and
  `Phase 214 attention status sync through Phase 213`.
- The aggregate phase range assertion uses `range(168, 215)`.
...
- The stale-negative checks and aggregate range must advance together: stale
  prefixes/markers move from 1-213 to 1-214 while the aggregate assertion moves
  from `range(168, 214)` to `range(168, 215)`.
- Previous Phase 213 wording and the Phase 214 accepted feature label must be
...
- The stale-marker guard must reject only standalone terminal `1-213` / `1–213`
  markers while preserving the valid `Phase 121-167` internal range and the
  Phase 213 and Phase 214 short labels.
- The controller must check allowed/prohibited path overlap before execution; any
  ...
  phase.
- This phase must not update Phase 214 or older accepted feature artifacts during
  S1.
