...
summary: >
  ...
  regression through accepted Phase 192, without changing runtime, source,
...
# Phase 193: CodeStable Attention Status Sync Through Phase 192
...
Phase 192 is accepted in the feature archive, so the mandatory startup status is
...
- the current-status bullet contains `Current status (2026-06-13): All phases 1-192 accepted`;
- the bullet names Phase 168 through Phase 192 with the same short labels above;
- the bullet preserves Phase 168 through Phase 191 labels and appends `Phase 192 attention status sync through Phase 191`;
- the bullet no longer contains `All phases 1-191 accepted`;
- the bullet no longer contains standalone stale terminal `1-191` / `1–191` markers;
...
Do not edit Phase 192 artifacts or any older accepted phase artifacts during S1.
S2 may edit only the new Phase 193 artifacts listed above.
...
- `design/codestable/attention.md` reports current status as 2026-06-13 with all
  phases 1-192 accepted.
- The current-status bullet preserves the existing Phase 168 through Phase 191
  short labels and appends `Phase 192 attention status sync through Phase 191`.
...
- The current-status bullet no longer reports Phase 191 as the terminal accepted
  phase.
- The status regression rejects `All phases 1-191 accepted`.
- The status regression rejects standalone stale terminal `1-191` / `1–191`
  markers.
...
- The stale-marker guard must reject only standalone terminal `1-191` / `1–191`
  markers while preserving the valid internal `Phase 121-167` range and the
  Phase 191 and Phase 192 short labels.
- This phase must not update Phase 192 or older accepted feature artifacts during
  S1.
