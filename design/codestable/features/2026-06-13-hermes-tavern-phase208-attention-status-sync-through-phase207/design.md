...
summary: >
  ...
  regression through accepted Phase 207, without changing runtime, source,
...
# Phase 208: CodeStable Attention Status Sync Through Phase 207
...
Phase 207 is accepted in the feature archive: its checklist has `status:
accepted` and `workflow_status: completed`, and `phase207-acceptance.md` has
`status: accepted`. No Phase 208 feature directory is present in the feature
...
- the current-status bullet contains `Current status (2026-06-13): All phases 1-207 accepted`;
- the bullet names Phase 168 through Phase 207 with the same short labels above;
- the bullet preserves Phase 168 through Phase 206 labels and appends `Phase 207 attention status sync through Phase 206`;
- the bullet no longer contains `All phases 1-206 accepted`;
- the bullet no longer contains standalone stale terminal `1-206` / `1–206` markers;
- the valid internal `Phase 121-167` range remains allowed;
- the final punctuation ends with `Phase 205 attention status sync through Phase 204, Phase 206 attention status sync through Phase 205, and Phase 207 attention status sync through Phase 206.`;
- the aggregate range assertion covers Phase 168 through Phase 207 with `range(168, 208)`;
...
Do not edit Phase 207 artifacts or any older accepted phase artifacts during S1.
S2 may edit only the new Phase 208 artifacts listed above.
...
Phase 1-207 terminal status, Phase 168-207 explicit labels, stale 1-206
rejection, valid Phase 121-167 preservation, final-list punctuation, and
`range(168, 208)` aggregate assertion.

S2 is controller-only closeout. It records acceptance evidence in
`phase208-acceptance.md` and finalizes the Phase 208 design/checklist statuses
...
- `PYTHONDONTWRITEBYTECODE=1 python design/codestable/tools/validate-yaml.py --file design/codestable/features/2026-06-13-hermes-tavern-phase208-attention-status-sync-through-phase207/design.md --require doc_type --require status --require feature`
- `PYTHONDONTWRITEBYTECODE=1 python design/codestable/tools/validate-yaml.py --file design/codestable/features/2026-06-13-hermes-tavern-phase208-attention-status-sync-through-phase207/checklist.yaml --yaml-only --require doc_type --require status --require feature`
...
- static marker guard must confirm: exactly one current-status line, terminal `1-207`, explicit Phase 168 through Phase 207 labels, `range(168, 208)`, final punctuation through Phase 207, stale terminal `1-206` rejection, valid `Phase 121-167` preservation, and no generated phase-discovery calls.
...
- `design/codestable/attention.md` reports current status as 2026-06-13 with all
  phases 1-207 accepted.
- The current-status bullet preserves the existing Phase 168 through Phase 206
  short labels and appends `Phase 207 attention status sync through Phase 206`.
- The final list punctuation is exactly:
  `Phase 205 attention status sync through Phase 204, Phase 206 attention status sync through Phase 205, and Phase 207 attention status sync through Phase 206.`
- The current-status bullet no longer reports Phase 206 as the terminal accepted
  phase.
- The status regression rejects `All phases 1-206 accepted`.
- The status regression rejects standalone stale terminal `1-206` / `1–206`
  markers.
- The status regression allows the valid `Phase 121-167` internal range.
- The status regression explicitly asserts Phase 168 through Phase 207 labels,
  including `Phase 207 attention status sync through Phase 206`.
- The aggregate phase range assertion uses `range(168, 208)`.
...
- The stale-negative checks and aggregate range must advance together: stale
  prefixes/markers move from 1-205 to 1-206 while the aggregate assertion moves
  from `range(168, 207)` to `range(168, 208)`.
- Previous Phase 206 wording and the Phase 207 accepted feature label must be
...
- The stale-marker guard must reject only standalone terminal `1-206` / `1–206`
  markers while preserving the valid `Phase 121-167` internal range and the
  Phase 206 and Phase 207 short labels.
- The controller must check allowed/prohibited path overlap before execution; any
  ...
  phase.
- This phase must not update Phase 207 or older accepted feature artifacts during
  S1.
