...
date: "2026-06-07"
summary: >
  Add a bounded local /rp card export <card> command that writes one stored
  character card as SillyTavern-compatible JSON and returns a quoted MEDIA marker
  without mutating runtime state or touching provider/generation paths.
