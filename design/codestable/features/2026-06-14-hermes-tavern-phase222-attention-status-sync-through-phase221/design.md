...
summary: >
  ...
  regression through accepted Phase 221, without changing runtime/source behavior
...
# Phase 222: CodeStable Attention Status Sync Through Phase 221
...
Phase 221 is accepted: the Phase 221 checklist has `status: accepted` and
`workflow_status: completed`, and `phase221-acceptance.md` has
`status: accepted`.

`design/codestable/attention.md` currently reports
`Current status (2026-06-14): All phases 1-220 accepted` and ends with
`Phase 218 attention status sync through Phase 217, Phase 219 attention status
sync through Phase 218, and Phase 220 attention status sync through Phase 219.`

`tests/test_hermes_tavern_codestable_status.py` currently guards terminal range
1-220, stale 1-219 markers, Phase 168 through Phase 220 labels, and
`range(168, 221)`.

A focused feature status scan found no `draft`, `planned`, `in-progress`,
`pending`, `todo`, `open`, `blocked`, `failed`, or `started` feature/checklist
status that should outrank this bounded metadata sync.

## Gap

`design/codestable/attention.md` is the mandatory startup read for CodeStable work
in this repository. Phase 221 is accepted in the feature archive, but the startup
status and focused static regression still stop at Phase 220. The mandatory
startup status and focused regression are therefore one accepted phase behind.
...
non-empty overlap blocks S1 and S2.
...
Do not edit Phase 221 artifacts or any older accepted phase artifacts during S1.
S2 may edit only the new Phase 222 artifacts listed above.

S1 must not mark final accepted statuses and must not create
`phase222-acceptance.md`. S2 is controller-only acceptance closeout.
...
Phase 1-221 terminal status, Phase 168-221 explicit labels, stale 1-220
rejection, valid Phase 121-167 preservation, final-list punctuation, and
`range(168, 222)` aggregate assertion.

S2 is controller-only closeout. It records acceptance evidence in
`phase222-acceptance.md` and finalizes the Phase 222 checklist/design statuses
...
- `PYTHONDONTWRITEBYTECODE=1 python design/codestable/tools/validate-yaml.py --file design/codestable/features/2026-06-14-hermes-tavern-phase222-attention-status-sync-through-phase221/design.md --require doc_type --require status --require feature --require implementation_ready`
- `PYTHONDONTWRITEBYTECODE=1 python design/codestable/tools/validate-yaml.py --file design/codestable/features/2026-06-14-hermes-tavern-phase222-attention-status-sync-through-phase221/checklist.yaml --yaml-only --require doc_type --require status --require feature --require implementation_ready`
...
- static marker guard must confirm: exactly one current-status line, terminal `1-221`, explicit Phase 168 through Phase 221 labels, `range(168, 222)`, final punctuation through Phase 221, stale terminal `1-220` / `1–220` rejection, valid `Phase 121-167` preservation, and no generated phase-discovery calls.
...
Phase 222 is acceptable when:

- the single current-status bullet reports all phases 1-221 accepted;
- the status bullet preserves prior labels through Phase 220 and appends only `Phase 221 attention status sync through Phase 220`;
- the final suffix is exactly `Phase 219 attention status sync through Phase 218, Phase 220 attention status sync through Phase 219, and Phase 221 attention status sync through Phase 220.`;
- stale terminal 1-220 markers are rejected while valid internal `Phase 121-167` remains allowed;
- `REQUIRED_PHASE_LABELS` includes Phase 168 through Phase 221;
- the aggregate assertion is `range(168, 222)`;
...
- The stale-negative checks and aggregate range must advance together: stale
  prefixes/markers move from 1-220 while the aggregate assertion moves to
  `range(168, 222)`.
- The current status line is long; keep it as one bullet and do not split it into
  generated summaries or multiple current-status bullets.
- The final-list punctuation must move the `and` from Phase 220 to Phase 221
  without rewriting older accepted labels.
