...
summary: >
  ...
  regression through accepted Phase 193, without changing runtime, source,
...
# Phase 194: CodeStable Attention Status Sync Through Phase 193
...
Phase 193 is accepted in the feature archive, so the mandatory startup status is
...
- the current-status bullet contains `Current status (2026-06-13): All phases 1-193 accepted`;
- the bullet names Phase 168 through Phase 193 with the same short labels above;
- the bullet preserves Phase 168 through Phase 192 labels and appends `Phase 193 attention status sync through Phase 192`;
- the bullet no longer contains `All phases 1-192 accepted`;
- the bullet no longer contains standalone stale terminal `1-192` / `1–192` markers;
...
Do not edit Phase 193 artifacts or any older accepted phase artifacts during S1.
S2 may edit only the new Phase 194 artifacts listed above.
...
- `design/codestable/attention.md` reports current status as 2026-06-13 with all
  phases 1-193 accepted.
- The current-status bullet preserves the existing Phase 168 through Phase 192
  short labels and appends `Phase 193 attention status sync through Phase 192`.
- The final list punctuation is exactly:
  `Phase 191 attention status sync through Phase 190, Phase 192 attention status sync through Phase 191, and Phase 193 attention status sync through Phase 192.`
- The current-status bullet no longer reports Phase 192 as the terminal accepted
  phase.
- The status regression rejects `All phases 1-192 accepted`.
- The status regression rejects standalone stale terminal `1-192` / `1–192`
  markers.
...
- The stale-marker guard must reject only standalone terminal `1-192` / `1–192`
  markers while preserving the valid internal `Phase 121-167` range and the
  Phase 192 and Phase 193 short labels.
- This phase must not update Phase 193 or older accepted feature artifacts during
  S1.
