...
summary: >
  ...
  regression through accepted Phase 185, without changing runtime, source,
...
# Phase 186: CodeStable Attention Status Sync Through Phase 185
...
Phase 185 is now accepted in the feature archive, so the mandatory startup status
...
- the current-status bullet contains `Current status (2026-06-12): All phases 1-185 accepted`;
- the bullet names Phase 168 through Phase 185 with the same short labels above;
- the bullet no longer contains standalone terminal `1-184` / `1–184` markers;
...
- `design/codestable/attention.md` reports current status as 2026-06-12 with all
  phases 1-185 accepted.
- The current-status bullet explicitly includes Phase 168 through Phase 185 short
  labels.
- The current-status bullet no longer reports Phase 184 as the terminal accepted
  phase.
- The status regression rejects standalone terminal `1-184` / `1–184` markers.
...
- The stale-marker guard must reject only standalone terminal `1-184` / `1–184`
  markers while preserving the valid internal `Phase 121-167` range and the
  Phase 184 and Phase 185 short labels.
