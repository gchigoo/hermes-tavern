...
summary: >
  ...
  regression through accepted Phase 188, without changing runtime, source,
...
# Phase 189: CodeStable Attention Status Sync Through Phase 188
...
Phase 188 is now accepted in the feature archive, so the mandatory startup status
...
- the current-status bullet contains `Current status (2026-06-12): All phases 1-188 accepted`;
- the bullet names Phase 168 through Phase 188 with the same short labels above;
- the bullet no longer contains standalone terminal `1-187` / `1–187` markers;
...
- `design/codestable/attention.md` reports current status as 2026-06-12 with all
  phases 1-188 accepted.
- The current-status bullet preserves the existing Phase 168 through Phase 187
  short labels and appends `Phase 188 attention status sync through Phase 187`.
- The current-status bullet no longer reports Phase 187 as the terminal accepted
  phase.
- The status regression rejects standalone terminal `1-187` / `1–187` markers.
...
- The stale-marker guard must reject only standalone terminal `1-187` / `1–187`
  markers while preserving the valid internal `Phase 121-167` range and the
  Phase 187 and Phase 188 short labels.
