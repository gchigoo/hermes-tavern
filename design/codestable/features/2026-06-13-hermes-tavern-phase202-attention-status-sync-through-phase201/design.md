...
status: approved
feature: "2026-06-13-hermes-tavern-phase202-attention-status-sync-through-phase201"
date: "2026-06-13"
summary: >
  Sync the mandatory CodeStable attention current-status line and focused static
  regression through accepted Phase 201, without changing runtime, source,
...
# Phase 202: CodeStable Attention Status Sync Through Phase 201
...
Phase 201 is accepted in the feature archive: its design is approved, its
checklist is accepted/completed, and `phase201-acceptance.md` has
`status: accepted`. The mandatory startup status and focused static regression
are therefore one accepted phase behind.
...
- the current-status bullet contains `Current status (2026-06-13): All phases 1-201 accepted`;
- the bullet names Phase 168 through Phase 201 with the same short labels above;
- the bullet preserves Phase 168 through Phase 200 labels and appends `Phase 201 attention status sync through Phase 200`;
- the bullet no longer contains `All phases 1-200 accepted`;
- the bullet no longer contains standalone stale terminal `1-200` / `1–200` markers;
- the valid internal `Phase 121-167` range remains allowed;
- the final punctuation ends with `Phase 199 attention status sync through Phase 198, Phase 200 attention status sync through Phase 199, and Phase 201 attention status sync through Phase 200.`;
- the aggregate range assertion covers Phase 168 through Phase 201 with `range(168, 202)`;
...
handling changes, safety-bypass behavior, or SillyTavern asset compatibility
changes.
...
- `design/codestable/features/2026-06-13-hermes-tavern-phase202-attention-status-sync-through-phase201/design.md`
- `design/codestable/features/2026-06-13-hermes-tavern-phase202-attention-status-sync-through-phase201/checklist.yaml`
- `design/codestable/features/2026-06-13-hermes-tavern-phase202-attention-status-sync-through-phase201/phase202-acceptance.md`
...
Do not edit Phase 201 artifacts or any older accepted phase artifacts during S1.
S2 may edit only the new Phase 202 artifacts listed above.
...
Phase 1-201 terminal status, Phase 168-201 explicit labels, stale 1-200
rejection, valid Phase 121-167 preservation, final-list punctuation, and
`range(168, 202)` aggregate assertion.

S2 is controller-only closeout. It records acceptance evidence in
`phase202-acceptance.md` and finalizes the Phase 202 design/checklist statuses
...
- `python -c "from fnmatch import fnmatch; allowed={'design/codestable/features/2026-06-13-hermes-tavern-phase202-attention-status-sync-through-phase201/design.md','design/codestable/features/2026-06-13-hermes-tavern-phase202-attention-status-sync-through-phase201/checklist.yaml','design/codestable/features/2026-06-13-hermes-tavern-phase202-attention-status-sync-through-phase201/phase202-acceptance.md','design/codestable/attention.md','tests/test_hermes_tavern_codestable_status.py'}; prohibited=['run_agent.py','cli.py','gateway/**','src/**','plugins/**','build/**','build/lib/**','README.md','design/HERMES_TAVERN_DESIGN.md','design/codestable/architecture/**','design/codestable/reference/**','design/codestable/roadmap/**','design/codestable/requirements/**','design/codestable/compound/**']; overlap=sorted((a,p) for a in allowed for p in prohibited if a==p or fnmatch(a,p) or (p.endswith('/**') and (a==p[:-3] or a.startswith(p[:-3] + '/')))); assert not overlap, overlap"`
- `PYTHONDONTWRITEBYTECODE=1 python design/codestable/tools/validate-yaml.py --file design/codestable/features/2026-06-13-hermes-tavern-phase202-attention-status-sync-through-phase201/design.md --require doc_type --require status --require feature`
- `PYTHONDONTWRITEBYTECODE=1 python design/codestable/tools/validate-yaml.py --file design/codestable/features/2026-06-13-hermes-tavern-phase202-attention-status-sync-through-phase201/checklist.yaml --yaml-only --require doc_type --require status --require feature`
...
- `PYTHONDONTWRITEBYTECODE=1 python -c "from pathlib import Path; import re; line=[l for l in Path('design/codestable/attention.md').read_text(encoding='utf-8').splitlines() if re.match(r'^\s*-\s*Current status ', l)]; assert len(line)==1; line=line[0]; test=Path('tests/test_hermes_tavern_codestable_status.py').read_text(encoding='utf-8'); labels=['Phase 168 image settings JSON export','Phase 169 project JSON export surface parity','Phase 170 export command-surface regression','Phase 171 root-design export parity','Phase 172 attention status sync through Phase 171','Phase 173 root-design section 17 import/export summary parity','Phase 174 attention status sync through Phase 173','Phase 175 attention status sync through Phase 174','Phase 176 attention status sync through Phase 175','Phase 177 attention status sync through Phase 176','Phase 178 attention status sync through Phase 177','Phase 179 attention status sync through Phase 178','Phase 180 attention status sync through Phase 179','Phase 181 attention status sync through Phase 180','Phase 182 attention status sync through Phase 181','Phase 183 attention status sync through Phase 182','Phase 184 attention status sync through Phase 183','Phase 185 attention status sync through Phase 184','Phase 186 attention status sync through Phase 185','Phase 187 attention status sync through Phase 186','Phase 188 attention status sync through Phase 187','Phase 189 attention status sync through Phase 188','Phase 190 attention status sync through Phase 189','Phase 191 attention status sync through Phase 190','Phase 192 attention status sync through Phase 191','Phase 193 attention status sync through Phase 192','Phase 194 attention status sync through Phase 193','Phase 195 attention status sync through Phase 194','Phase 196 attention status sync through Phase 195','Phase 197 attention status sync through Phase 196','Phase 198 attention status sync through Phase 197','Phase 199 attention status sync through Phase 198','Phase 200 attention status sync through Phase 199','Phase 201 attention status sync through Phase 200']; assert 'Current status (2026-06-13): All phases 1-201 accepted' in line; assert 'All phases 1-200 accepted' not in line; assert re.search(r'(?<!\d)1-200(?!\d)', line) is None; assert re.search(r'(?<!\d)1–200(?!\d)', line) is None; assert 'Phase 121-167' in line; assert line.endswith('Phase 199 attention status sync through Phase 198, Phase 200 attention status sync through Phase 199, and Phase 201 attention status sync through Phase 200.'); assert all(f'Phase {phase} ' in line for phase in range(168, 202)); missing_line=[label for label in labels if label not in line]; missing_test=[label for label in labels if label not in test]; assert not missing_line, missing_line; assert not missing_test, missing_test; assert 'Current status (2026-06-13): All phases 1-201 accepted' in test; assert 'All phases 1-200 accepted' in test; assert '1-200' in test; assert '1–200' in test; assert 'range(168, 202)' in test; assert '.glob(' not in test; assert '.rglob(' not in test; assert 'iterdir(' not in test; assert 'os.walk' not in test"`
- `python -c "import subprocess; allowed={'design/codestable/features/2026-06-13-hermes-tavern-phase202-attention-status-sync-through-phase201/design.md','design/codestable/features/2026-06-13-hermes-tavern-phase202-attention-status-sync-through-phase201/checklist.yaml','design/codestable/features/2026-06-13-hermes-tavern-phase202-attention-status-sync-through-phase201/phase202-acceptance.md','design/codestable/attention.md','tests/test_hermes_tavern_codestable_status.py'}; changed=set(subprocess.check_output(['git','diff','--name-only','HEAD'], text=True).splitlines()); untracked=set(subprocess.check_output(['git','ls-files','--others','--exclude-standard'], text=True).splitlines()); extra=sorted(p for p in changed | untracked if p and p not in allowed); assert not extra, extra"`
...
- `PYTHONDONTWRITEBYTECODE=1 python design/codestable/tools/validate-yaml.py --file design/codestable/features/2026-06-13-hermes-tavern-phase202-attention-status-sync-through-phase201/phase202-acceptance.md --require doc_type --require status --require feature`

## Acceptance

- `design/codestable/attention.md` reports current status as 2026-06-13 with all
  phases 1-201 accepted.
- The current-status bullet preserves the existing Phase 168 through Phase 200
  short labels and appends `Phase 201 attention status sync through Phase 200`.
- The final list punctuation is exactly:
  `Phase 199 attention status sync through Phase 198, Phase 200 attention status sync through Phase 199, and Phase 201 attention status sync through Phase 200.`
- The current-status bullet no longer reports Phase 200 as the terminal accepted
  phase.
- The status regression rejects `All phases 1-200 accepted`.
- The status regression rejects standalone stale terminal `1-200` / `1–200`
  markers.
- The status regression allows the valid `Phase 121-167` internal range.
- The status regression explicitly asserts Phase 168 through Phase 201 labels,
  including `Phase 201 attention status sync through Phase 200`.
- The aggregate phase range assertion uses `range(168, 202)`.
...
- The stale-marker guard must reject only standalone terminal `1-200` / `1–200`
  markers while preserving the valid `Phase 121-167` internal range and the
  Phase 200 and Phase 201 short labels.
- The controller must check allowed/prohibited path overlap before execution; any
  ...
  phase.
- This phase must not update Phase 201 or older accepted feature artifacts during
  S1.
