...
summary: >
  ...
  regression through accepted Phase 206, without changing runtime, source,
...
# Phase 207: CodeStable Attention Status Sync Through Phase 206
...
Phase 206 is accepted in the feature archive: its checklist has `status:
accepted` and `workflow_status: completed`, and `phase206-acceptance.md` has
`status: accepted`. No Phase 207 feature directory is present in the feature
...
- the current-status bullet contains `Current status (2026-06-13): All phases 1-206 accepted`;
- the bullet names Phase 168 through Phase 206 with the same short labels above;
- the bullet preserves Phase 168 through Phase 205 labels and appends `Phase 206 attention status sync through Phase 205`;
- the bullet no longer contains `All phases 1-205 accepted`;
- the bullet no longer contains standalone stale terminal `1-205` / `1–205` markers;
- the valid internal `Phase 121-167` range remains allowed;
- the final punctuation ends with `Phase 204 attention status sync through Phase 203, Phase 205 attention status sync through Phase 204, and Phase 206 attention status sync through Phase 205.`;
- the aggregate range assertion covers Phase 168 through Phase 206 with `range(168, 207)`;
...
Do not edit Phase 206 artifacts or any older accepted phase artifacts during S1.
S2 may edit only the new Phase 207 artifacts listed above.
...
Phase 1-206 terminal status, Phase 168-206 explicit labels, stale 1-205
rejection, valid Phase 121-167 preservation, final-list punctuation, and
`range(168, 207)` aggregate assertion.

S2 is controller-only closeout. It records acceptance evidence in
`phase207-acceptance.md` and finalizes the Phase 207 design/checklist statuses
...
- static marker guard must confirm: exactly one current-status line, terminal `1-206`, explicit Phase 168 through Phase 206 labels, `range(168, 207)`, final punctuation through Phase 206, stale terminal `1-205` rejection, valid `Phase 121-167` preservation, and no generated phase-discovery calls.
...
- `design/codestable/attention.md` reports current status as 2026-06-13 with all
  phases 1-206 accepted.
- The current-status bullet preserves the existing Phase 168 through Phase 205
  short labels and appends `Phase 206 attention status sync through Phase 205`.
...
- The current-status bullet no longer reports Phase 205 as the terminal accepted
  phase.
- The status regression rejects `All phases 1-205 accepted`.
- The status regression rejects standalone stale terminal `1-205` / `1–205`
  markers.
- The status regression allows the valid `Phase 121-167` internal range.
- The status regression explicitly asserts Phase 168 through Phase 206 labels,
  including `Phase 206 attention status sync through Phase 205`.
- The aggregate phase range assertion uses `range(168, 207)`.
...
- The stale-negative checks and aggregate range must advance together: stale
  prefixes/markers move from 1-204 to 1-205 while the aggregate assertion moves
  from `range(168, 206)` to `range(168, 207)`.
- Previous Phase 205 wording and the Phase 206 accepted feature label must be
...
- The stale-marker guard must reject only standalone terminal `1-205` / `1–205`
  markers while preserving the valid `Phase 121-167` internal range and the
  Phase 205 and Phase 206 short labels.
- The controller must check allowed/prohibited path overlap before execution; any
  ...
  phase.
- This phase must not update Phase 206 or older accepted feature artifacts during
  S1.
