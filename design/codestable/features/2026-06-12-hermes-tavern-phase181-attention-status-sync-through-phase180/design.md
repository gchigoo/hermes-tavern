...
summary: >
  ...
  regression through accepted Phase 180, without changing runtime, source,
...
# Phase 181: CodeStable Attention Status Sync Through Phase 180
...
Phase 180 is now accepted in the feature archive, so the mandatory startup status
...
- the current-status bullet contains `Current status (2026-06-12): All phases 1-180 accepted`;
- the bullet names Phase 168 through Phase 180 with the same short labels above;
- the bullet no longer contains standalone terminal `1-179` / `1–179` markers;
...
- `design/codestable/attention.md` reports current status as 2026-06-12 with all
  phases 1-180 accepted.
- The current-status bullet explicitly includes Phase 168 through Phase 180 short
  labels.
- The current-status bullet no longer reports Phase 179 as the terminal accepted
  phase.
