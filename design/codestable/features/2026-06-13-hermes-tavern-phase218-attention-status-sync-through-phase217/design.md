...
summary: >
  ...
  regression through accepted Phase 217, without changing runtime, source,
...
# Phase 218: CodeStable Attention Status Sync Through Phase 217
...
1-216 accepted` and ends with `Phase 216 attention status sync through Phase
215`.

Phase 217 is accepted in the feature archive:
...
`phase217-acceptance.md` has `status: accepted`. The focused status regression
currently guards Phase 1-216 with stale 1-215 markers and `range(168, 217)`. The
...
- the current-status bullet contains `Current status (2026-06-13): All phases 1-217 accepted`;
- the bullet names Phase 168 through Phase 217 with the same short labels above;
- the bullet preserves Phase 168 through Phase 216 labels and appends `Phase 217 attention status sync through Phase 216`;
- the bullet no longer contains `All phases 1-216 accepted`;
- the bullet no longer contains standalone stale terminal `1-216` / `1–216` markers;
- the valid `Phase 121-167` internal range remains allowed;
- the final punctuation ends with `Phase 215 attention status sync through Phase 214, Phase 216 attention status sync through Phase 215, and Phase 217 attention status sync through Phase 216.`;
- the aggregate phase range assertion covers Phase 168 through Phase 217 with `range(168, 218)`;
...
Do not edit Phase 217 artifacts or any older accepted phase artifacts during S1.
S2 may edit only the new Phase 218 artifacts listed above.
...
Phase 1-217 terminal status, Phase 168-217 explicit labels, stale 1-216
rejection, valid Phase 121-167 preservation, final-list punctuation, and
`range(168, 218)` aggregate assertion.

S2 is controller-only closeout. It records acceptance evidence in
`phase218-acceptance.md` and finalizes the Phase 218 design/checklist statuses
...
- `PYTHONDONTWRITEBYTECODE=1 python design/codestable/tools/validate-yaml.py --file design/codestable/features/2026-06-13-hermes-tavern-phase218-attention-status-sync-through-phase217/design.md --require doc_type --require status --require feature --require implementation_ready`
- `PYTHONDONTWRITEBYTECODE=1 python design/codestable/tools/validate-yaml.py --file design/codestable/features/2026-06-13-hermes-tavern-phase218-attention-status-sync-through-phase217/checklist.yaml --yaml-only --require doc_type --require status --require feature --require implementation_ready`
...
- static marker guard must confirm: exactly one current-status line, terminal `1-217`, explicit Phase 168 through Phase 217 labels, `range(168, 218)`, final punctuation through Phase 217, stale terminal `1-216` / `1–216` rejection, valid `Phase 121-167` preservation, and no generated phase-discovery calls.
...
- `design/codestable/attention.md` reports current status as 2026-06-13 with all
  phases 1-217 accepted.
- The current-status bullet preserves the existing Phase 168 through Phase 216
  short labels and appends `Phase 217 attention status sync through Phase 216`.
- The final list punctuation is exactly:
  `Phase 215 attention status sync through Phase 214, Phase 216 attention status sync through Phase 215, and Phase 217 attention status sync through Phase 216.`
- The current-status bullet no longer reports Phase 216 as the terminal accepted
  phase.
- The status regression rejects `All phases 1-216 accepted`.
- The status regression rejects standalone stale terminal `1-216` / `1–216`
  markers.
- The status regression allows the valid `Phase 121-167` internal range.
- The status regression explicitly asserts Phase 168 through Phase 217 labels,
  including `Phase 216 attention status sync through Phase 215` and
  `Phase 217 attention status sync through Phase 216`.
- The aggregate phase range assertion uses `range(168, 218)`.
...
- The stale-negative checks and aggregate range must advance together: stale
  prefixes/markers move from 1-216 to 1-217 while the aggregate assertion moves
  from `range(168, 217)` to `range(168, 218)`.
- Previous Phase 216 wording and the Phase 217 accepted feature label must be
...
- The stale-marker guard must reject only standalone terminal `1-216` / `1–216`
  markers while preserving the valid `Phase 121-167` internal range and the
  Phase 216 and Phase 217 short labels.
- The controller must check allowed/prohibited path overlap before execution; any
  ...
  phase.
- This phase must not update Phase 217 or older accepted feature artifacts during
  S1.
