...
summary: >
  ...
  regression through accepted Phase 211, without changing runtime, source,
...
# Phase 212: CodeStable Attention Status Sync Through Phase 211

## Gap

`design/codestable/attention.md` is the mandatory startup read for CodeStable work
in this repository. The controller and Codex architect confirmed it currently
reports `Current status (2026-06-13): All phases 1-210 accepted` and ends with
`Phase 210 attention status sync through Phase 209`.

Phase 211 is accepted in the feature archive:
...
`phase211-acceptance.md` has `status: accepted`. The focused status regression
currently guards Phase 1-210 with `REQUIRED_PHASE_LABELS` through Phase 210 and
`range(168, 211)`. The mandatory startup status and focused static regression are
...
- the current-status bullet contains `Current status (2026-06-13): All phases 1-211 accepted`;
- the bullet names Phase 168 through Phase 211 with the same short labels above;
- the bullet preserves Phase 168 through Phase 210 labels and appends `Phase 211 attention status sync through Phase 210`;
- the bullet no longer contains `All phases 1-210 accepted`;
- the bullet no longer contains standalone stale terminal `1-210` / `1–210` markers;
- the valid `Phase 121-167` internal range remains allowed;
- the final punctuation ends with `Phase 209 attention status sync through Phase 208, Phase 210 attention status sync through Phase 209, and Phase 211 attention status sync through Phase 210.`;
- the aggregate range assertion covers Phase 168 through Phase 211 with `range(168, 212)`;
...
Do not edit Phase 211 artifacts or any older accepted phase artifacts during S1.
S2 may edit only the new Phase 212 artifacts listed above.
...
Phase 1-211 terminal status, Phase 168-211 explicit labels, stale 1-210
rejection, valid Phase 121-167 preservation, final-list punctuation, and
`range(168, 212)` aggregate assertion.

S2 is controller-only closeout. It records acceptance evidence in
`phase212-acceptance.md` and finalizes the Phase 212 design/checklist statuses
...
- `PYTHONDONTWRITEBYTECODE=1 python design/codestable/tools/validate-yaml.py --file design/codestable/features/2026-06-13-hermes-tavern-phase212-attention-status-sync-through-phase211/design.md --require doc_type --require status --require feature`
- `PYTHONDONTWRITEBYTECODE=1 python design/codestable/tools/validate-yaml.py --file design/codestable/features/2026-06-13-hermes-tavern-phase212-attention-status-sync-through-phase211/checklist.yaml --yaml-only --require doc_type --require status --require feature`
...
- static marker guard must confirm: exactly one current-status line, terminal `1-211`, explicit Phase 168 through Phase 211 labels, `range(168, 212)`, final punctuation through Phase 211, stale terminal `1-210` / `1–210` rejection, valid `Phase 121-167` preservation, and no generated phase-discovery calls.
...
- `design/codestable/attention.md` reports current status as 2026-06-13 with all
  phases 1-211 accepted.
- The current-status bullet preserves the existing Phase 168 through Phase 210
  short labels and appends `Phase 211 attention status sync through Phase 210`.
- The final list punctuation is exactly:
  `Phase 209 attention status sync through Phase 208, Phase 210 attention status sync through Phase 209, and Phase 211 attention status sync through Phase 210.`
- The current-status bullet no longer reports Phase 210 as the terminal accepted
  phase.
- The status regression rejects `All phases 1-210 accepted`.
- The status regression rejects standalone stale terminal `1-210` / `1–210`
  markers.
- The status regression allows the valid `Phase 121-167` internal range.
- The status regression explicitly asserts Phase 168 through Phase 211 labels,
  including `Phase 211 attention status sync through Phase 210`.
- The aggregate phase range assertion uses `range(168, 212)`.
...
- The stale-negative checks and aggregate range must advance together: stale
  prefixes/markers move from 1-209 to 1-210 while the aggregate assertion moves
  from `range(168, 211)` to `range(168, 212)`.
- Previous Phase 210 wording and the Phase 211 accepted feature label must be
...
- The stale-marker guard must reject only standalone terminal `1-210` / `1–210`
  markers while preserving the valid `Phase 121-167` internal range and the
  Phase 210 and Phase 211 short labels.
- The controller must check allowed/prohibited path overlap before execution; any
  ...
  phase.
- This phase must not update Phase 211 or older accepted feature artifacts during
  S1.
