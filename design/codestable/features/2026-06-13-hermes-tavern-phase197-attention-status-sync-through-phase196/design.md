...
summary: >
  ...
  regression through accepted Phase 196, without changing runtime, source,
...
# Phase 197: CodeStable Attention Status Sync Through Phase 196
...
Phase 196 is accepted in the feature archive, with accepted checklist and
`phase196-acceptance.md` artifacts. The mandatory startup status and focused
...
- the current-status bullet contains `Current status (2026-06-13): All phases 1-196 accepted`;
- the bullet names Phase 168 through Phase 196 with the same short labels above;
- the bullet preserves Phase 168 through Phase 195 labels and appends `Phase 196 attention status sync through Phase 195`;
- the bullet no longer contains `All phases 1-195 accepted`;
- the bullet no longer contains standalone stale terminal `1-195` / `1–195` markers;
- the valid internal `Phase 121-167` range remains allowed;
- the final punctuation ends with `Phase 194 attention status sync through Phase 193, Phase 195 attention status sync through Phase 194, and Phase 196 attention status sync through Phase 195.`;
- the aggregate range assertion covers Phase 168 through Phase 196 with `range(168, 197)`;
...
Do not edit Phase 196 artifacts or any older accepted phase artifacts during S1.
S2 may edit only the new Phase 197 artifacts listed above.
...
- `PYTHONDONTWRITEBYTECODE=1 python design/codestable/tools/validate-yaml.py --file design/codestable/features/2026-06-13-hermes-tavern-phase197-attention-status-sync-through-phase196/design.md --require doc_type --require status --require feature`
- `PYTHONDONTWRITEBYTECODE=1 python design/codestable/tools/validate-yaml.py --file design/codestable/features/2026-06-13-hermes-tavern-phase197-attention-status-sync-through-phase196/checklist.yaml --yaml-only --require doc_type --require status --require feature`
...
- `design/codestable/attention.md` reports current status as 2026-06-13 with all
  phases 1-196 accepted.
- The current-status bullet preserves the existing Phase 168 through Phase 195
  short labels and appends `Phase 196 attention status sync through Phase 195`.
- The final list punctuation is exactly:
  `Phase 194 attention status sync through Phase 193, Phase 195 attention status sync through Phase 194, and Phase 196 attention status sync through Phase 195.`
- The current-status bullet no longer reports Phase 195 as the terminal accepted
  phase.
- The status regression rejects `All phases 1-195 accepted`.
- The status regression rejects standalone stale terminal `1-195` / `1–195`
  markers.
- The status regression allows the valid `Phase 121-167` internal range.
- The status regression explicitly asserts Phase 168 through Phase 196 labels,
  including `Phase 196 attention status sync through Phase 195`.
- The aggregate phase range assertion uses `range(168, 197)`.
...
- The stale-marker guard must reject only standalone terminal `1-195` / `1–195`
  markers while preserving the valid `Phase 121-167` internal range and the
  Phase 195 and Phase 196 short labels.
- This phase must not update Phase 196 or older accepted feature artifacts during
  S1.
