...
summary: >
  ...
  regression through accepted Phase 273, without changing runtime/source behavior,
...
# Phase 274: CodeStable Attention Status Sync Through Phase 273

## Read-Only Architect Precheck

- Architect JSONL: `/tmp/hermes-tavern-architect-phase274-1781574610.jsonl`.
- `design/codestable/attention.md` currently reports `Current status (2026-06-15): All phases 1-272 accepted`.
- `tests/test_hermes_tavern_codestable_status.py` currently guards labels through Phase 272, stale terminal `1-271` markers, final suffix through Phase 272, and `range(168, 273)`.
- Phase 273 is accepted as a docs/test/status-only sync through Phase 272.
- No Phase 274 feature directory existed before this artifact.
...
- Current prefix becomes: `Current status (2026-06-15): All phases 1-273 accepted`
- Reject stale prefix/standalone markers: `1-272` and `1–272`
- Preserve valid internal marker: `Phase 121-167`
- Preserve all existing Phase 168-272 short labels.
- Append new terminal label exactly: `Phase 273 attention status sync through Phase 272`
- Final suffix exactly: `Phase 271 attention status sync through Phase 270, Phase 272 attention status sync through Phase 271, and Phase 273 attention status sync through Phase 272.`
- Do not add Phase 274 to the attention status line.

## Test Contract

- `CURRENT_STATUS_PREFIX = "Current status (2026-06-15): All phases 1-273 accepted"`
- `STALE_STATUS_PREFIX = "Current status (2026-06-15): All phases 1-272 accepted"`
- `STALE_PHASE_MARKER = "1-272"`
- `STALE_PHASE_MARKER_EN_DASH = "1–272"`
- `REQUIRED_PHASE_LABELS` includes all existing labels and `Phase 273 attention status sync through Phase 272`.
- `FINAL_STATUS_SUFFIX` is the exact suffix above.
- `phase_range` uses `range(168, 274)`.
- Source assertion requires literal `range(168, 274)` and rejects stale `range(168, 273)` via split-string construction.
...
- `PYTHONDONTWRITEBYTECODE=1 python design/codestable/tools/validate-yaml.py --dir design/codestable/features/2026-06-16-hermes-tavern-phase274-attention-status-sync-through-phase273 --require doc_type --require status --require feature`
...
S1 is ready for parent/controller closeout when the single current-status bullet reports all phases `1-273` accepted, preserves `Phase 121-167` and all Phase 168-272 labels, appends exactly the Phase 273 label, ends with the exact suffix through Phase 273, rejects stale `1-272` / `1–272`, and the focused static regression remains explicit and passes. S1 must leave `acceptance.md` and final S2 closeout to the parent/controller.
