...
title: "Phase 391 attention/status sync through Phase 390"
status: approved
...
summary: "Approved S1 design for Phase 391 attention/status sync through accepted Phase 390; executor implementation and parent verification remain non-final."
tags: [hermes-tavern, codestable, attention, status-sync, docs, tests, phase391, company-boost, parent-handoff]
---
...
Phase 391 is the correct next bounded CodeStable slice.

Evidence confirmed by the read-only architect pass at `/tmp/hermes-tavern-architect_design-20260630-170621.jsonl`:
...
- That status line currently starts with `Current status (2026-06-18): All phases 1-390 accepted`.
- Its terminal suffix is `Phase 388 attention status sync through Phase 387, Phase 389 attention status sync through Phase 388, and Phase 390 attention status sync through Phase 389.`
- Phase 390 artifacts are complete and accepted: `design.md` is approved, `checklist.yaml` is accepted, and `acceptance.md` is accepted under `design/codestable/features/2026-06-30-hermes-tavern-phase390-attention-status-sync-through-phase389/`.
- `tests/test_hermes_tavern_codestable_status.py` is pinned to Phase 390: `CURRENT_STATUS_PREFIX` is 1-390, stale prefix/markers are 1-389, `FINAL_STATUS_SUFFIX` ends through Phase 390, and live aggregate range is `range(168, 391)`.
...
- No Phase 391 feature artifact existed at architect read time.

## Goals

- Materialize the Phase 391 design/checklist pair for a company-boost S1 parent-handoff slice.
- Advance `design/codestable/attention.md` exactly one status tick from 1-390 to 1-391.
- Append `Phase 391 attention status sync through Phase 390` exactly once in the attention current-status line.
- Preserve `Phase 121-167` and every explicit Phase 168 through Phase 390 label.
- Update the focused status regression to guard the Phase 391 live status contract.
...
- Current prefix advances from `Current status (2026-06-18): All phases 1-390 accepted` to `Current status (2026-06-18): All phases 1-391 accepted`.
- Append exactly once in the attention status line: `Phase 391 attention status sync through Phase 390`.
- Preserve `Phase 121-167` and every explicit Phase 168 through Phase 390 label.
...
- Final suffix becomes: `Phase 389 attention status sync through Phase 388, Phase 390 attention status sync through Phase 389, and Phase 391 attention status sync through Phase 390.`

### tests/test_hermes_tavern_codestable_status.py

- `CURRENT_STATUS_PREFIX = "Current status (2026-06-18): All phases 1-391 accepted"`
- `STALE_STATUS_PREFIX = "Current status (2026-06-18): All phases 1-390 accepted"`
- `STALE_PHASE_MARKER = "1-390"`
- `STALE_PHASE_MARKER_EN_DASH = "1–390"`
- Append `"Phase 391 attention status sync through Phase 390"` to `REQUIRED_PHASE_LABELS`.
- Preserve every existing Phase 168 through Phase 390 required label.
- `FINAL_STATUS_SUFFIX = "Phase 389 attention status sync through Phase 388, Phase 390 attention status sync through Phase 389, and Phase 391 attention status sync through Phase 390."`
- `phase_range = range(168, 392)`
- `aggregate_range = "range(168, 392)"`
- Add `stale_aggregate_range_391 = "".join(["range(168, ", "39", "1", ")"])`.
- Include `stale_aggregate_range_391` in the stale assertion tuple.
- Preserve prior split stale guards, including `stale_aggregate_range_390` and earlier.
- Replace the final split `stale_aggregate_guard` so it builds stale `range(168, 391)` without embedding contiguous `range(168, 391)` in the test source.
- Do not leave a contiguous stale literal `range(168, 391)` in the test source after the update; the live range is `range(168, 392)`.
...
Static checks must verify the exact current-status prefix, stale prefix, stale hyphen/en-dash markers, final suffix, preservation of `Phase 121-167`, preservation of every explicit Phase 168 through Phase 390 label, append of Phase 391 exactly once in the attention status line, current-status section placement, anchored assignment-count guards, no direct stale `range(168, 391)` literal in the focused test source, and no discovery tokens.
...
- Static Phase 391 status/scope guard using changed plus untracked files.
...
- The terminal `and` must move to Phase 391; Phase 390 should become comma-separated.
- `range(168, 391)` becomes stale and must not remain as a contiguous stale literal in the focused test source.
- Existing split stale guards, especially `stale_aggregate_range_390`, must remain present in the stale tuple.
