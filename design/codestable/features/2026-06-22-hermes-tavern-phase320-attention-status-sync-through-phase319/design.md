...
summary: >
  ...
  regression through accepted Phase 319 only, without changing runtime behavior.
---

# Phase 320: CodeStable Attention Status Sync Through Phase 319
...
- `design/codestable/attention.md` reports `Current status (2026-06-18): All phases 1-319 accepted`.
- `tests/test_hermes_tavern_codestable_status.py` enforces the Phase 319 prefix, required labels through Phase 319, `phase_range = range(168, 320)`, and `aggregate_range = "range(168, 320)"`.
- Recent sibling status-sync artifacts through Phase 319 are accepted.
- No Phase 320 status-sync feature directory exists before parent-controller materialization.
- Within the requested inspection scope, no higher-confidence unfinished bounded artifact is present for this tick.

Selected slice: recurring status-only Phase 320 sync through accepted Phase 319.

## Goals

- Advance exactly one recurring status-sync phase: Phase 320 syncs through accepted Phase 319 only.
- Update only `attention.md`, the focused static status test, and the new Phase 320 feature artifact directory.
...
- Leave checklist/acceptance finalization, staging, commit, push, messages, service lifecycle commands, and progress-state updates outside executor S1.
...
- Do not add a Phase 321 attention/status label; this slice records Phase 320 as the newly accepted completed phase.
...
- From: `Current status (2026-06-18): All phases 1-319 accepted`
- To: `Current status (2026-06-18): All phases 1-320 accepted`

Append exactly once after `Phase 319 attention status sync through Phase 318`:
- Convert `Phase 318 attention status sync through Phase 317, and Phase 319 attention status sync through Phase 318.`
- To `Phase 318 attention status sync through Phase 317, Phase 319 attention status sync through Phase 318, and Phase 320 attention status sync through Phase 319.`

Preserve:
- `Phase 121-167`
- Every explicit `Phase 168` through `Phase 319` short label exactly as already written.

Do not add:
- Any `Phase 321` attention/status label to `attention.md` or the focused status label list.

Final attention suffix must end with:
`Phase 318 attention status sync through Phase 317, Phase 319 attention status sync through Phase 318, and Phase 320 attention status sync through Phase 319.`

Full focused-test suffix must become exactly:
`Phase 303 attention status sync through Phase 302, Phase 304 attention status sync through Phase 303, Phase 305 attention status sync through Phase 304, Phase 306 attention status sync through Phase 305, Phase 307 attention status sync through Phase 306, Phase 308 attention status sync through Phase 307, Phase 309 attention status sync through Phase 308, Phase 310 attention status sync through Phase 309, Phase 311 attention status sync through Phase 310, Phase 312 attention status sync through Phase 311, Phase 313 attention status sync through Phase 312, Phase 314 attention status sync through Phase 313, Phase 315 attention status sync through Phase 314, Phase 316 attention status sync through Phase 315, Phase 317 attention status sync through Phase 316, Phase 318 attention status sync through Phase 317, Phase 319 attention status sync through Phase 318, and Phase 320 attention status sync through Phase 319.`

## Focused Test Contract

Update `tests/test_hermes_tavern_codestable_status.py`:
- `CURRENT_STATUS_PREFIX = "Current status (2026-06-18): All phases 1-320 accepted"`
- `STALE_STATUS_PREFIX = "Current status (2026-06-18): All phases 1-319 accepted"`
- `STALE_PHASE_MARKER = "1-319"`
- `STALE_PHASE_MARKER_EN_DASH = "1–319"`
- Append `Phase 320 attention status sync through Phase 319` to `REQUIRED_PHASE_LABELS` exactly once.
- Preserve every existing required label through Phase 319.
- `FINAL_STATUS_SUFFIX` equals the Phase 303 through Phase 320 suffix above, including the final period.
- Runtime `phase_range` becomes `range(168, 321)`.
- Source self-check literal becomes `aggregate_range = "range(168, 321)"`.
- Add split stale aggregate guard for `range(168, 320)`.
- Retain older split stale aggregate guards, including `range(168, 319)`, `range(168, 318)`, and earlier existing guards.
...
- `design/codestable/features/2026-06-22-hermes-tavern-phase320-attention-status-sync-through-phase319/design.md`
- `design/codestable/features/2026-06-22-hermes-tavern-phase320-attention-status-sync-through-phase319/checklist.yaml`
- `design/codestable/features/2026-06-22-hermes-tavern-phase320-attention-status-sync-through-phase319/acceptance.md`
...
- Parent may create only the new Phase 320 feature artifact directory.
...
- Update only `attention.md` prefix and append only the Phase 320 label.
...
- Finalize only the Phase 320 feature artifacts after verification.
- If any gate fails, leave status non-final and report the failing command.

## Verification Commands

- `PYTHONDONTWRITEBYTECODE=1 python design/codestable/tools/validate-yaml.py --dir design/codestable/features/2026-06-22-hermes-tavern-phase320-attention-status-sync-through-phase319 --require doc_type --require status --require feature`
...
- Static guard for stale standalone `1-319` patterns in `attention.md` and intentional-only stale values in `tests/test_hermes_tavern_codestable_status.py`.
- Static guard for no Phase 321 attention/status label in `attention.md` or the focused status label list.
...
- Leaving stale `1-319` markers in the attention current-status bullet.
...
Rollback point: revert only the Phase 320 allowed files to the pre-executor state; because executor must not stage, commit, push, finalize acceptance, or update progress state, rollback is limited to the dirty working tree before parent closeout.

## Executor Prompt

Implement only Phase 320 attention/status sync through accepted Phase 319. Edit only `design/codestable/attention.md` and `tests/test_hermes_tavern_codestable_status.py`. Advance current status to `All phases 1-320 accepted`, preserve `Phase 121-167` and every explicit Phase 168 through Phase 319 label, append exactly `Phase 320 attention status sync through Phase 319`, and do not add any Phase 321 attention/status label. Update focused test constants, suffix, required labels, `range(168, 321)`, `aggregate_range = "range(168, 321)"`, stale markers for `All phases 1-319 accepted`, and split stale aggregate guard for `range(168, 320)` while retaining older stale guards including `range(168, 319)` and `range(168, 318)`. Preserve no-discovery-token guards and the single `CURRENT_STATUS_PREFIX` assignment guard. Do not commit, push, stage, update state, send messages, touch runtime/protected paths, change dependencies, edit prior phase directories, finalize checklist/acceptance, or run service lifecycle commands.
