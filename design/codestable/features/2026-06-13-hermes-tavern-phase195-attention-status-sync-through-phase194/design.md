...
summary: >
  ...
  regression through accepted Phase 194, without changing runtime, source,
...
# Phase 195: CodeStable Attention Status Sync Through Phase 194
...
Phase 194 is accepted in the feature archive, so the mandatory startup status is
...
- the current-status bullet contains `Current status (2026-06-13): All phases 1-194 accepted`;
- the bullet names Phase 168 through Phase 194 with the same short labels above;
- the bullet preserves Phase 168 through Phase 193 labels and appends `Phase 194 attention status sync through Phase 193`;
- the bullet no longer contains `All phases 1-193 accepted`;
- the bullet no longer contains standalone stale terminal `1-193` / `1–193` markers;
- the valid internal `Phase 121-167` range remains allowed;
- the final punctuation ends with `Phase 192 attention status sync through Phase 191, Phase 193 attention status sync through Phase 192, and Phase 194 attention status sync through Phase 193.`;
...
Do not edit Phase 194 artifacts or any older accepted phase artifacts during S1.
S2 may edit only the new Phase 195 artifacts listed above.
...
- `design/codestable/attention.md` reports current status as 2026-06-13 with all
  phases 1-194 accepted.
- The current-status bullet preserves the existing Phase 168 through Phase 193
  short labels and appends `Phase 194 attention status sync through Phase 193`.
...
- The current-status bullet no longer reports Phase 193 as the terminal accepted
  phase.
- The status regression rejects `All phases 1-193 accepted`.
- The status regression rejects standalone stale terminal `1-193` / `1–193`
  markers.
...
- The stale-marker guard must reject only standalone terminal `1-193` / `1–193`
  markers while preserving the valid `Phase 121-167` internal range and the
  Phase 193 and Phase 194 short labels.
- This phase must not update Phase 194 or older accepted feature artifacts during
  S1.
