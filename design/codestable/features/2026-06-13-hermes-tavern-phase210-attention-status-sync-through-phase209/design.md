...
status: approved
feature: "2026-06-13-hermes-tavern-phase210-attention-status-sync-through-phase209"
date: "2026-06-13"
summary: >
  Sync the mandatory CodeStable attention current-status line and focused static
  regression through accepted Phase 209, without changing runtime, source,
  plugin, provider, prompt/generation, import/export, schema, root-design,
  architecture, reference, README, roadmap, requirement, compound, build, or
...
# Phase 210: CodeStable Attention Status Sync Through Phase 209

## Gap

`design/codestable/attention.md` is the mandatory startup read for CodeStable work
in this repository. Read-only inspection confirms it currently reports `Current
status (2026-06-13): All phases 1-208 accepted` and ends with `Phase 208
attention status sync through Phase 207`.

Phase 209 is accepted in the feature archive:
...
`phase209-acceptance.md` has `status: accepted`. No Phase 210 feature directory
currently exists. The mandatory startup status and focused static regression are
therefore one accepted phase behind.

This is CodeStable startup-context metadata drift, not a runtime, source, plugin,
provider, prompt/generation, import/export, schema, root-design, architecture,
reference, README, roadmap, requirement, compound, build, or Hermes core feature
gap.
...
- the current-status bullet contains `Current status (2026-06-13): All phases 1-209 accepted`;
- the bullet names Phase 168 through Phase 209 with the same short labels above;
- the bullet preserves Phase 168 through Phase 208 labels and appends `Phase 209 attention status sync through Phase 208`;
- the bullet no longer contains `All phases 1-208 accepted`;
- the bullet no longer contains standalone stale terminal `1-208` / `1–208` markers;
- the valid internal `Phase 121-167` range remains allowed;
- the final punctuation ends with `Phase 207 attention status sync through Phase 206, Phase 208 attention status sync through Phase 207, and Phase 209 attention status sync through Phase 208.`;
- the aggregate range assertion covers Phase 168 through Phase 209 with `range(168, 210)`;
...
- `design/codestable/features/2026-06-13-hermes-tavern-phase210-attention-status-sync-through-phase209/design.md`
- `design/codestable/features/2026-06-13-hermes-tavern-phase210-attention-status-sync-through-phase209/checklist.yaml`
- `design/codestable/features/2026-06-13-hermes-tavern-phase210-attention-status-sync-through-phase209/phase210-acceptance.md`
...
Do not edit Phase 209 artifacts or any older accepted phase artifacts during S1.
S2 may edit only the new Phase 210 artifacts listed above.
...
Phase 1-209 terminal status, Phase 168-209 explicit labels, stale 1-208
rejection, valid Phase 121-167 preservation, final-list punctuation, and
`range(168, 210)` aggregate assertion.

S2 is controller-only closeout. It records acceptance evidence in
`phase210-acceptance.md` and finalizes the Phase 210 design/checklist statuses
...
- `PYTHONDONTWRITEBYTECODE=1 python design/codestable/tools/validate-yaml.py --file design/codestable/features/2026-06-13-hermes-tavern-phase210-attention-status-sync-through-phase209/design.md --require doc_type --require status --require feature`
- `PYTHONDONTWRITEBYTECODE=1 python design/codestable/tools/validate-yaml.py --file design/codestable/features/2026-06-13-hermes-tavern-phase210-attention-status-sync-through-phase209/checklist.yaml --yaml-only --require doc_type --require status --require feature`
...
- static marker guard must confirm: exactly one current-status line, terminal `1-209`, explicit Phase 168 through Phase 209 labels, `range(168, 210)`, final punctuation through Phase 209, stale terminal `1-208` / `1–208` rejection, valid `Phase 121-167` preservation, and no generated phase-discovery calls.
...
- `design/codestable/attention.md` reports current status as 2026-06-13 with all
  phases 1-209 accepted.
- The current-status bullet preserves the existing Phase 168 through Phase 208
  short labels and appends `Phase 209 attention status sync through Phase 208`.
- The final list punctuation is exactly:
  `Phase 207 attention status sync through Phase 206, Phase 208 attention status sync through Phase 207, and Phase 209 attention status sync through Phase 208.`
- The current-status bullet no longer reports Phase 208 as the terminal accepted
  phase.
- The status regression rejects `All phases 1-208 accepted`.
- The status regression rejects standalone stale terminal `1-208` / `1–208`
  markers.
- The status regression allows the valid `Phase 121-167` internal range.
- The status regression explicitly asserts Phase 168 through Phase 209 labels,
  including `Phase 209 attention status sync through Phase 208`.
- The aggregate phase range assertion uses `range(168, 210)`.
...
- The stale-negative checks and aggregate range must advance together: stale
  prefixes/markers move from 1-207 to 1-208 while the aggregate assertion moves
  from `range(168, 209)` to `range(168, 210)`.
- Previous Phase 208 wording and the Phase 209 accepted feature label must be
...
- The stale-marker guard must reject only standalone terminal `1-208` / `1–208`
  markers while preserving the valid `Phase 121-167` internal range and the
  Phase 208 and Phase 209 short labels.
- The controller must check allowed/prohibited path overlap before execution; any
  ...
  phase.
- This phase must not update Phase 209 or older accepted feature artifacts during
  S1.
