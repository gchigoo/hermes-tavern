...
title: "Phase 423 attention/status sync through Phase 422"
status: approved
...
summary: "Approved S1 design for Phase 423 attention/status sync through accepted Phase 422; executor work is limited to status docs/tests plus non-final checklist handoff, with parent verification retaining acceptance/commit authority."
...
- Branch `main` is at `00b02d0` (`docs: accept tavern phase 422 status sync`), with the controller-confirmed tree clean.
- Phase 422 is accepted at HEAD, with accepted `design.md`, `checklist.yaml`, and `acceptance.md` under `design/codestable/features/2026-07-02-hermes-tavern-phase422-attention-status-sync-through-phase421/`.
- No Phase 423 feature directory existed at architect read time.
- `design/codestable/attention.md` is the startup attention source of truth and currently says `Current status (2026-06-18): All phases 1-422 accepted`.
- `tests/test_hermes_tavern_codestable_status.py` currently guards the Phase 422 live status contract with current/stale prefixes for 422/421, required labels through Phase 422, terminal suffix through Phase 420/421/422, the live aggregate range ending at 423, and split stale guards through `stale_aggregate_range_422`.

## Goals

- Materialize the Phase 423 feature `design.md` and `checklist.yaml` for this bounded company-boost S1 pass.
- Advance `design/codestable/attention.md` exactly one status tick from phases 1-422 accepted to phases 1-423 accepted.
- Append `Phase 423 attention status sync through Phase 422` exactly once in the attention current-status line.
...
- Update `tests/test_hermes_tavern_codestable_status.py` to enforce the Phase 423 live status contract, including the new live range and stale split guards.
...
- Change the current prefix from `Current status (2026-06-18): All phases 1-422 accepted` to `Current status (2026-06-18): All phases 1-423 accepted`.
- Append exactly once: `Phase 423 attention status sync through Phase 422`.
...
- End the status line with exactly: `Phase 421 attention status sync through Phase 420, Phase 422 attention status sync through Phase 421, and Phase 423 attention status sync through Phase 422.`

### tests/test_hermes_tavern_codestable_status.py

- `CURRENT_STATUS_PREFIX = "Current status (2026-06-18): All phases 1-423 accepted"`
- `STALE_STATUS_PREFIX = "Current status (2026-06-18): All phases 1-422 accepted"`
- `STALE_PHASE_MARKER = "1-422"`
- `STALE_PHASE_MARKER_EN_DASH = "1–422"`
- Append `"Phase 423 attention status sync through Phase 422"` to `REQUIRED_PHASE_LABELS` after Phase 422.
- Preserve every existing required label, including the nonuniform Phase 168, 169, 170, 171, and 173 labels.
- `FINAL_STATUS_SUFFIX = "Phase 421 attention status sync through Phase 420, Phase 422 attention status sync through Phase 421, and Phase 423 attention status sync through Phase 422."`
- `phase_range = range(168, 424)`
- `aggregate_range = "range(168, 424)"`
- Add `stale_aggregate_range_423 = "".join(["range(168, ", "42", "3", ")"])` and include it in the stale tuple.
- Preserve the prior split guards `stale_aggregate_range_422` and `stale_aggregate_range_421`, and ensure the direct stale aggregate literals ending at 423, 422, and 421 are rejected without embedding those stale direct literals contiguously in the test source.
...
- `PYTHONPYCACHEPREFIX=/tmp/hermes-tavern-pycache-phase423-executor python -m py_compile tests/test_hermes_tavern_codestable_status.py`
