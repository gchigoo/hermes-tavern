...
summary: >
  ...
  regression through accepted Phase 223, without changing runtime/source behavior
...
# Phase 224: CodeStable Attention Status Sync Through Phase 223
...
Phase 223 is accepted: `phase223-acceptance.md` has `status: accepted`, and
the accepted report confirms Phase 223 synced the startup status and focused
static regression through accepted Phase 222.

`design/codestable/attention.md` currently reports
`Current status (2026-06-14): All phases 1-222 accepted` and ends with
`Phase 220 attention status sync through Phase 219, Phase 221 attention status
sync through Phase 220, and Phase 222 attention status sync through Phase 221.`

`tests/test_hermes_tavern_codestable_status.py` currently guards terminal range
1-222, stale 1-221 markers, Phase 168 through Phase 222 labels, and
`range(168, 223)`.

The target Phase 224 feature directory did not exist at read-only precheck time.

## Gap

`design/codestable/attention.md` is the mandatory startup read for CodeStable work
in this repository. Phase 223 is accepted in the feature archive, but the startup
status and focused static regression still stop at Phase 222. The mandatory
...
Do not edit Phase 223 artifacts or any older accepted phase artifacts during S1.
S2 may edit only the new Phase 224 artifacts listed above.

S1 must not mark final accepted statuses and must not create
`phase224-acceptance.md`. S2 is controller-only acceptance closeout.
...
Phase 1-223 terminal status, Phase 168-223 explicit labels, stale 1-222
rejection, valid Phase 121-167 preservation, final-list punctuation, and
`range(168, 224)` aggregate assertion.

S2 is controller-only closeout. It records acceptance evidence in
`phase224-acceptance.md` and finalizes the Phase 224 checklist/design statuses
...
- `PYTHONDONTWRITEBYTECODE=1 python design/codestable/tools/validate-yaml.py --file design/codestable/features/2026-06-14-hermes-tavern-phase224-attention-status-sync-through-phase223/design.md --require doc_type --require status --require feature --require implementation_ready`
- `PYTHONDONTWRITEBYTECODE=1 python design/codestable/tools/validate-yaml.py --file design/codestable/features/2026-06-14-hermes-tavern-phase224-attention-status-sync-through-phase223/checklist.yaml --yaml-only --require doc_type --require status --require feature --require implementation_ready`
...
- static marker guard must confirm: exactly one current-status line, terminal `1-223`, explicit Phase 168 through Phase 223 labels, `range(168, 224)`, final punctuation through Phase 223, stale terminal `1-222` / `1–222` rejection, valid `Phase 121-167` preservation, and no generated phase-discovery calls.
...
Phase 224 is acceptable when:

- the single current-status bullet reports all phases 1-223 accepted;
- the status bullet preserves prior labels through Phase 222 and appends exactly `Phase 223 attention status sync through Phase 222`;
- the final suffix is exactly `Phase 221 attention status sync through Phase 220, Phase 222 attention status sync through Phase 221, and Phase 223 attention status sync through Phase 222.`;
- stale terminal 1-222 markers are rejected while valid internal `Phase 121-167` remains allowed;
- `REQUIRED_PHASE_LABELS` includes Phase 168 through Phase 223;
- the aggregate assertion is `range(168, 224)`;
...
- The stale-negative checks and aggregate range must advance together: stale
  prefixes/markers move from 1-221 to 1-222 while the aggregate assertion moves
  from `range(168, 223)` to `range(168, 224)`.
...
- The final-list punctuation must move the `and` from Phase 222 to Phase 223
