...
summary: >
  ...
  regression through accepted Phase 216, without changing runtime, source,
...
# Phase 217: CodeStable Attention Status Sync Through Phase 216
...
1-215 accepted` and ends with `Phase 215 attention status sync through Phase
214`.

Phase 216 is accepted in the feature archive:
...
`phase216-acceptance.md` has `status: accepted`. The focused status regression
currently guards Phase 1-215 with stale 1-214 markers and `range(168, 216)`. The
...
`design/codestable/features/2026-06-13-hermes-tavern-phase217-attention-status-sync-through-phase216/`

Update only the current-status bullet in `design/codestable/attention.md` so it
reports accepted phases through Phase 216.
...
- the current-status bullet contains `Current status (2026-06-13): All phases 1-216 accepted`;
- the bullet names Phase 168 through Phase 216 with the same short labels above;
- the bullet preserves Phase 168 through Phase 215 labels and appends `Phase 216 attention status sync through Phase 215`;
- the bullet no longer contains `All phases 1-215 accepted`;
- the bullet no longer contains standalone stale terminal `1-215` / `1–215` markers;
- the valid `Phase 121-167` internal range remains allowed;
- the final punctuation ends with `Phase 214 attention status sync through Phase 213, Phase 215 attention status sync through Phase 214, and Phase 216 attention status sync through Phase 215.`;
- the aggregate phase range assertion covers Phase 168 through Phase 216 with `range(168, 217)`;
...
Do not edit Phase 216 artifacts or any older accepted phase artifacts during S1.
S2 may edit only the new Phase 217 artifacts listed above.
...
Phase 1-216 terminal status, Phase 168-216 explicit labels, stale 1-215
rejection, valid Phase 121-167 preservation, final-list punctuation, and
`range(168, 217)` aggregate assertion.

S2 is controller-only closeout. It records acceptance evidence in
`phase217-acceptance.md` and finalizes the Phase 217 design/checklist statuses
...
- `PYTHONDONTWRITEBYTECODE=1 python design/codestable/tools/validate-yaml.py --file design/codestable/features/2026-06-13-hermes-tavern-phase217-attention-status-sync-through-phase216/design.md --require doc_type --require status --require feature --require implementation_ready`
- `PYTHONDONTWRITEBYTECODE=1 python design/codestable/tools/validate-yaml.py --file design/codestable/features/2026-06-13-hermes-tavern-phase217-attention-status-sync-through-phase216/checklist.yaml --yaml-only --require doc_type --require status --require feature --require implementation_ready`
...
- static marker guard must confirm: exactly one current-status line, terminal `1-216`, explicit Phase 168 through Phase 216 labels, `range(168, 217)`, final punctuation through Phase 216, stale terminal `1-215` / `1–215` rejection, valid `Phase 121-167` preservation, and no generated phase-discovery calls.
...
- `design/codestable/attention.md` reports current status as 2026-06-13 with all
  phases 1-216 accepted.
- The current-status bullet preserves the existing Phase 168 through Phase 215
  short labels and appends `Phase 216 attention status sync through Phase 215`.
- The final list punctuation is exactly:
  `Phase 214 attention status sync through Phase 213, Phase 215 attention status sync through Phase 214, and Phase 216 attention status sync through Phase 215.`
- The current-status bullet no longer reports Phase 215 as the terminal accepted
  phase.
- The status regression rejects `All phases 1-215 accepted`.
- The status regression rejects standalone stale terminal `1-215` / `1–215`
  markers.
- The status regression allows the valid `Phase 121-167` internal range.
- The status regression explicitly asserts Phase 168 through Phase 216 labels,
  including `Phase 215 attention status sync through Phase 214` and
  `Phase 216 attention status sync through Phase 215`.
- The aggregate phase range assertion uses `range(168, 217)`.
...
- The stale-negative checks and aggregate range must advance together: stale
  prefixes/markers move from 1-215 to 1-216 while the aggregate assertion moves
  from `range(168, 216)` to `range(168, 217)`.
- Previous Phase 215 wording and the Phase 216 accepted feature label must be
...
- The stale-marker guard must reject only standalone terminal `1-215` / `1–215`
  markers while preserving the valid `Phase 121-167` internal range and the
  Phase 215 and Phase 216 short labels.
- The controller must check allowed/prohibited path overlap before execution; any
  ...
  phase.
- This phase must not update Phase 216 or older accepted feature artifacts during
  S1.
