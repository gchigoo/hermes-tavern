...
summary: >
  ...
  regression through accepted Phase 186, without changing runtime, source,
...
# Phase 187: CodeStable Attention Status Sync Through Phase 186
...
Phase 186 is now accepted in the feature archive, so the mandatory startup status
...
reports accepted phases through Phase 186.
...
- the current-status bullet contains `Current status (2026-06-12): All phases 1-186 accepted`;
- the bullet names Phase 168 through Phase 186 with the same short labels above;
- the bullet no longer contains standalone terminal `1-185` / `1–185` markers;
...
- `design/codestable/attention.md` reports current status as 2026-06-12 with all
  phases 1-186 accepted.
- The current-status bullet explicitly includes Phase 168 through Phase 186 short
  labels.
- The current-status bullet no longer reports Phase 185 as the terminal accepted
  phase.
- The status regression rejects standalone terminal `1-185` / `1–185` markers.
...
- The stale-marker guard must reject only standalone terminal `1-185` / `1–185`
  markers while preserving the valid internal `Phase 121-167` range and the
  Phase 185 and Phase 186 short labels.
