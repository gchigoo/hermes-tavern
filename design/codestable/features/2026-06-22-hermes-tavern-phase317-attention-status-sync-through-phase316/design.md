...
summary: >
  ...
  regression through accepted Phase 316 only, without changing runtime behavior.
---

# Phase 317: CodeStable Attention Status Sync Through Phase 316
...
- `design/codestable/attention.md` reports `Current status (2026-06-18): All phases 1-316 accepted`.
- `tests/test_hermes_tavern_codestable_status.py` enforces the Phase 316 prefix and `range(168, 317)`.
- No Phase 317 status-sync feature directory existed before this controller materialization.

## Goals

- Advance exactly one recurring status-sync phase: Phase 317 syncs through accepted Phase 316 only.
- Update only `attention.md`, the focused static status test, and this Phase 317 feature artifact directory.
...
- Leave verification, checklist finalization, acceptance finalization, commit, push, CI watch, and progress-state update to the parent controller.
...
- Do not add a Phase 318 attention/status label; this slice records Phase 317 as the newly accepted completed phase.
...
- From: `Current status (2026-06-18): All phases 1-316 accepted`
- To: `Current status (2026-06-18): All phases 1-317 accepted`

Append exactly once after `Phase 316 attention status sync through Phase 315`:
- Convert `and Phase 316 attention status sync through Phase 315.`
- To `Phase 316 attention status sync through Phase 315, and Phase 317 attention status sync through Phase 316.`

Preserve:
- `Phase 121-167`
- Every explicit `Phase 168` through `Phase 316` short label exactly as already written.

Do not add:
- Any `Phase 318` attention/status label to `attention.md` or the focused status label list.

Final suffix must become exactly:
`Phase 303 attention status sync through Phase 302, Phase 304 attention status sync through Phase 303, Phase 305 attention status sync through Phase 304, Phase 306 attention status sync through Phase 305, Phase 307 attention status sync through Phase 306, Phase 308 attention status sync through Phase 307, Phase 309 attention status sync through Phase 308, Phase 310 attention status sync through Phase 309, Phase 311 attention status sync through Phase 310, Phase 312 attention status sync through Phase 311, Phase 313 attention status sync through Phase 312, Phase 314 attention status sync through Phase 313, Phase 315 attention status sync through Phase 314, Phase 316 attention status sync through Phase 315, and Phase 317 attention status sync through Phase 316.`

## Focused Test Contract

Update `tests/test_hermes_tavern_codestable_status.py`:
- `CURRENT_STATUS_PREFIX = "Current status (2026-06-18): All phases 1-317 accepted"`
- `STALE_STATUS_PREFIX = "Current status (2026-06-18): All phases 1-316 accepted"`
- `STALE_PHASE_MARKER = "1-316"`
- `STALE_PHASE_MARKER_EN_DASH = "1–316"`
- Append `Phase 317 attention status sync through Phase 316` to `REQUIRED_PHASE_LABELS` exactly once.
- Preserve every existing required label through Phase 316.
- `FINAL_STATUS_SUFFIX` equals the Phase 303/304/305/306/307/308/309/310/311/312/313/314/315/316/317 suffix above.
- Runtime `phase_range` becomes `range(168, 318)`.
- Source self-check literal becomes `aggregate_range = "range(168, 318)"`.
- Add split stale aggregate guard for `range(168, 317)`.
- Retain older split stale guards including 316, 315, 314, 313, 312, 311, 310, 309, 308, 307, 306, 305, 304, 303, 302, 301, 300, 299, 298, 297, 296, and earlier existing guards through 281.
...
- `design/codestable/features/2026-06-22-hermes-tavern-phase317-attention-status-sync-through-phase316/design.md`
- `design/codestable/features/2026-06-22-hermes-tavern-phase317-attention-status-sync-through-phase316/checklist.yaml`
- `design/codestable/features/2026-06-22-hermes-tavern-phase317-attention-status-sync-through-phase316/acceptance.md`
...
- Update only `attention.md` prefix and append only the Phase 317 label.
...
- `attention.md` must not contain stale terminal status `All phases 1-316 accepted` after the update.
- `attention.md` must not contain standalone stale markers `1-316` or `1–316`.
- Test source may contain stale values only as intentional `STALE_*` constants/assertions or split stale aggregate guards.
- Add split stale aggregate guard for `range(168, 317)` and retain older split guards for 316 through 281.

## Verification Commands

- `PYTHONDONTWRITEBYTECODE=1 python design/codestable/tools/validate-yaml.py --dir design/codestable/features/2026-06-22-hermes-tavern-phase317-attention-status-sync-through-phase316 --require doc_type --require status --require feature`
...
- Static guard for stale standalone `1-316` patterns in `attention.md` and `tests/test_hermes_tavern_codestable_status.py`.
...
- Leaving stale `1-316` markers in the attention current-status bullet.
...
Rollback point: revert only the Phase 317 allowed files to the pre-executor state; because executor must not stage, commit, or push, rollback is limited to the dirty working tree before parent closeout.

## Executor Prompt

Implement only Phase 317 attention/status sync through accepted Phase 316. Edit only `design/codestable/attention.md` and `tests/test_hermes_tavern_codestable_status.py`. Advance current status to `All phases 1-317 accepted`, preserve `Phase 121-167` and every explicit Phase 168 through Phase 316 label, append exactly `Phase 317 attention status sync through Phase 316`, and do not add any Phase 318 attention/status label. Update focused test constants, suffix, required labels, `range(168, 318)`, `aggregate_range = "range(168, 318)"`, stale markers for `All phases 1-316 accepted`, and split stale aggregate guard for `range(168, 317)`. Do not commit, push, stage, update state, send messages, touch runtime/protected paths, change dependencies, edit prior phase directories, finalize checklist/acceptance, or run service lifecycle commands.
