...
summary: >
  ...
  regression through accepted Phase 210, without changing runtime, source,
...
# Phase 211: CodeStable Attention Status Sync Through Phase 210

## Gap

`design/codestable/attention.md` is the mandatory startup read for CodeStable work
in this repository. The controller already read it and confirmed it currently
reports `Current status (2026-06-13): All phases 1-209 accepted` and ends with
`Phase 209 attention status sync through Phase 208`.

Phase 210 is accepted in the feature archive:
...
`phase210-acceptance.md` has `status: accepted`. The focused status regression
currently guards Phase 1-209 with `REQUIRED_PHASE_LABELS` through Phase 209 and
`range(168, 210)`. The mandatory startup status and focused static regression are
...
- the current-status bullet contains `Current status (2026-06-13): All phases 1-210 accepted`;
- the bullet names Phase 168 through Phase 210 with the same short labels above;
- the bullet preserves Phase 168 through Phase 209 labels and appends `Phase 210 attention status sync through Phase 209`;
- the bullet no longer contains `All phases 1-209 accepted`;
- the bullet no longer contains standalone stale terminal `1-209` / `1–209` markers;
- the valid internal `Phase 121-167` range remains allowed;
- the final punctuation ends with `Phase 208 attention status sync through Phase 207, Phase 209 attention status sync through Phase 208, and Phase 210 attention status sync through Phase 209.`;
- the aggregate range assertion covers Phase 168 through Phase 210 with `range(168, 211)`;
...
Do not edit Phase 210 artifacts or any older accepted phase artifacts during S1.
S2 may edit only the new Phase 211 artifacts listed above.
...
Phase 1-210 terminal status, Phase 168-210 explicit labels, stale 1-209
rejection, valid Phase 121-167 preservation, final-list punctuation, and
`range(168, 211)` aggregate assertion.

S2 is controller-only closeout. It records acceptance evidence in
`phase211-acceptance.md` and finalizes the Phase 211 design/checklist statuses
...
- `PYTHONDONTWRITEBYTECODE=1 python design/codestable/tools/validate-yaml.py --file design/codestable/features/2026-06-13-hermes-tavern-phase211-attention-status-sync-through-phase210/design.md --require doc_type --require status --require feature`
- `PYTHONDONTWRITEBYTECODE=1 python design/codestable/tools/validate-yaml.py --file design/codestable/features/2026-06-13-hermes-tavern-phase211-attention-status-sync-through-phase210/checklist.yaml --yaml-only --require doc_type --require status --require feature`
...
- static marker guard must confirm: exactly one current-status line, terminal `1-210`, explicit Phase 168 through Phase 210 labels, `range(168, 211)`, final punctuation through Phase 210, stale terminal `1-209` / `1–209` rejection, valid `Phase 121-167` preservation, and no generated phase-discovery calls.
...
- `design/codestable/attention.md` reports current status as 2026-06-13 with all
  phases 1-210 accepted.
- The current-status bullet preserves the existing Phase 168 through Phase 209
  short labels and appends `Phase 210 attention status sync through Phase 209`.
- The final list punctuation is exactly:
  `Phase 208 attention status sync through Phase 207, Phase 209 attention status sync through Phase 208, and Phase 210 attention status sync through Phase 209.`
- The current-status bullet no longer reports Phase 209 as the terminal accepted
  phase.
- The status regression rejects `All phases 1-209 accepted`.
- The status regression rejects standalone stale terminal `1-209` / `1–209`
  markers.
- The status regression allows the valid `Phase 121-167` internal range.
- The status regression explicitly asserts Phase 168 through Phase 210 labels,
  including `Phase 210 attention status sync through Phase 209`.
- The aggregate phase range assertion uses `range(168, 211)`.
...
- The stale-negative checks and aggregate range must advance together: stale
  prefixes/markers move from 1-208 to 1-209 while the aggregate assertion moves
  from `range(168, 210)` to `range(168, 211)`.
- Previous Phase 209 wording and the Phase 210 accepted feature label must be
...
- The stale-marker guard must reject only standalone terminal `1-209` / `1–209`
  markers while preserving the valid `Phase 121-167` internal range and the
  Phase 209 and Phase 210 short labels.
- The controller must check allowed/prohibited path overlap before execution; any
  ...
  phase.
- This phase must not update Phase 210 or older accepted feature artifacts during
  S1.
