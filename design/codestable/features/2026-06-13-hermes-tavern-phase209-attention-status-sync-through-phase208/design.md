...
summary: >
  ...
  regression through accepted Phase 208, without changing runtime, source,
...
# Phase 209: CodeStable Attention Status Sync Through Phase 208
...
Phase 208 is accepted in the feature archive:
...
`phase208-acceptance.md` has `status: accepted`. The mandatory startup status and
focused static regression are therefore one accepted phase behind.
...
- the current-status bullet contains `Current status (2026-06-13): All phases 1-208 accepted`;
- the bullet names Phase 168 through Phase 208 with the same short labels above;
- the bullet preserves Phase 168 through Phase 207 labels and appends `Phase 208 attention status sync through Phase 207`;
- the bullet no longer contains `All phases 1-207 accepted`;
- the bullet no longer contains standalone stale terminal `1-207` / `1–207` markers;
- the valid internal `Phase 121-167` range remains allowed;
- the final punctuation ends with `Phase 206 attention status sync through Phase 205, Phase 207 attention status sync through Phase 206, and Phase 208 attention status sync through Phase 207.`;
- the aggregate range assertion covers Phase 168 through Phase 208 with `range(168, 209)`;
...
Do not edit Phase 208 artifacts or any older accepted phase artifacts during S1.
S2 may edit only the new Phase 209 artifacts listed above.
...
Phase 1-208 terminal status, Phase 168-208 explicit labels, stale 1-207
rejection, valid Phase 121-167 preservation, final-list punctuation, and
`range(168, 209)` aggregate assertion.

S2 is controller-only closeout. It records acceptance evidence in
`phase209-acceptance.md` and finalizes the Phase 209 design/checklist statuses
...
- `PYTHONDONTWRITEBYTECODE=1 python design/codestable/tools/validate-yaml.py --file design/codestable/features/2026-06-13-hermes-tavern-phase209-attention-status-sync-through-phase208/design.md --require doc_type --require status --require feature`
- `PYTHONDONTWRITEBYTECODE=1 python design/codestable/tools/validate-yaml.py --file design/codestable/features/2026-06-13-hermes-tavern-phase209-attention-status-sync-through-phase208/checklist.yaml --yaml-only --require doc_type --require status --require feature`
...
- static marker guard must confirm: exactly one current-status line, terminal `1-208`, explicit Phase 168 through Phase 208 labels, `range(168, 209)`, final punctuation through Phase 208, stale terminal `1-207` / `1–207` rejection, valid `Phase 121-167` preservation, and no generated phase-discovery calls.
...
- `design/codestable/attention.md` reports current status as 2026-06-13 with all
  phases 1-208 accepted.
- The current-status bullet preserves the existing Phase 168 through Phase 207
  short labels and appends `Phase 208 attention status sync through Phase 207`.
- The final list punctuation is exactly:
  `Phase 206 attention status sync through Phase 205, Phase 207 attention status sync through Phase 206, and Phase 208 attention status sync through Phase 207.`
- The current-status bullet no longer reports Phase 207 as the terminal accepted
  phase.
- The status regression rejects `All phases 1-207 accepted`.
- The status regression rejects standalone stale terminal `1-207` / `1–207`
  markers.
- The status regression allows the valid `Phase 121-167` internal range.
- The status regression explicitly asserts Phase 168 through Phase 208 labels,
  including `Phase 208 attention status sync through Phase 207`.
- The aggregate phase range assertion uses `range(168, 209)`.
...
- The stale-negative checks and aggregate range must advance together: stale
  prefixes/markers move from 1-206 to 1-207 while the aggregate assertion moves
  from `range(168, 208)` to `range(168, 209)`.
- Previous Phase 207 wording and the Phase 208 accepted feature label must be
...
- The stale-marker guard must reject only standalone terminal `1-207` / `1–207`
  markers while preserving the valid `Phase 121-167` internal range and the
  Phase 207 and Phase 208 short labels.
- The controller must check allowed/prohibited path overlap before execution; any
  ...
  phase.
- This phase must not update Phase 208 or older accepted feature artifacts during
  S1.
