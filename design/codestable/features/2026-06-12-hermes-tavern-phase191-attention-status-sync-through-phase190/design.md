...
summary: >
  ...
  regression through accepted Phase 190, without changing runtime, source,
...
# Phase 191: CodeStable Attention Status Sync Through Phase 190
...
Phase 190 is now accepted in the feature archive, so the mandatory startup status
...
- the current-status bullet contains `Current status (2026-06-12): All phases 1-190 accepted`;
- the bullet names Phase 168 through Phase 190 with the same short labels above;
- the bullet preserves Phase 168 through Phase 189 labels and appends `Phase 190 attention status sync through Phase 189`;
- the bullet no longer contains standalone terminal `1-189` / `1–189` markers or `All phases 1-189 accepted`;
...
S2 acceptance/writeback is a later controller closeout step unless the controller
explicitly finalizes after verification. That closeout may use only:
...
`build/lib/**`, `README.md`, `design/HERMES_TAVERN_DESIGN.md`,
...
- `design/codestable/attention.md` reports current status as 2026-06-12 with all
  phases 1-190 accepted.
- The current-status bullet preserves the existing Phase 168 through Phase 189
  short labels and appends `Phase 190 attention status sync through Phase 189`.
- The current-status bullet no longer reports Phase 189 as the terminal accepted
  phase.
- The status regression rejects standalone terminal `1-189` / `1–189` markers.
- The status regression allows the valid `Phase 121-167` internal range.
- The status regression remains static and explicit, with no generated phase discovery.
...
- S2 acceptance/writeback remains controller-only unless explicitly finalized
...
- The stale-marker guard must reject only standalone terminal `1-189` / `1–189`
  markers while preserving the valid internal `Phase 121-167` range and the
  Phase 189 and Phase 190 short labels.
...
- This phase must not expand into runtime, source, plugin, provider,
  ...
  minors/underage, adult-fiction/RP compatibility, or safety behavior.
